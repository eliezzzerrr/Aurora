...
#property version   "7.34"   // file is TrendEMA_EA_v7.34 - bump both together
...
void DrawPanel()
  {
   ...
   //--- v7.34: thickness is appended because it is the chop filter input the
   //    user has to choose a number for, and reading it off the two edges by
   //    hand is exactly the kind of arithmetic the panel exists to remove.
   //    Marked THIN only when the filter is armed and would actually block.
   double thickNow = CloudThicknessPips();
   string thickTxt = StringFormat("  %.0fp", thickNow);
   if(MinCloudPips > 0.0 && gCloud != CLOUD_UNKNOWN && thickNow < MinCloudPips)
      thickTxt += " THIN";
   PanelRow(r++, "15M CLOUD",
            StringFormat("%s  %s/%s%s", cloudTxt,
