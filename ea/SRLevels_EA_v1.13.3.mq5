...
#property version   "1.133"  // file is SRLevels_EA_v1.13.3 - bump both together
...
enum ENUM_TREND_MODE
  {
   ...
   TREND_EMA_ONLY        // Price vs the EMA alone (the coach's red EMA9 line) - v1.6 default
  };
...
//    with it.
//--- v1.6, 10 Sep 2026, 16:18: the operator asked for the EMA, not the
//    cloud, to decide the side. 4H close above EMA9 = buy limits only,
//    below = sell limits only. Equal (rare) = nothing. The cloud is no
//    longer consulted for direction; it still contributes level points.
//    EMA9 on 4H flips every day or two, and every flip cancels the
//    resting limits on the old side (CancelOnTrendFlip), so this trades
//    the side of the current swing rather than the larger trend - which is
//    closer to what the coach's charts show than either the cloud gate or
//    no gate. No tester result for this mode yet.
input ENUM_TREND_MODE TrendMode = TREND_EMA_ONLY; // How the 4H picks a side
input int    TrendMAPeriod      = 50;    // MA50 on the trend TF (SMA, like the coach's blue line)
input int    TrendEMAPeriod     = 9;     // EMA9 (the coach's red line)
//--- v1.8, 14 Sep 2026: TREND_EMA_ONLY reads the EMA on ITS OWN timeframe,
//    not on TrendTF. The coach's post that morning named the invalidation
//    explicitly: "Gold needs to trade back above EMA9 in the Daily chart
//    (currently 4,380) ... to initially invalidate the Head and Shoulders".
//    The 4H EMA9 sat at 4,346.89 that same minute, 33.00 lower, so the EA
//    would have flipped to BUY on a bounce he was still calling a
//    downtrend. Set PERIOD_H4 here to restore v1.6-v1.7 behaviour.
//    Read at shift 1 like every other trend input, so the side changes on
//    a CLOSED daily candle and cannot repaint intraday - which also means
//    it changes at most once a day.
input ENUM_TIMEFRAMES TrendEMATF = PERIOD_D1; // TF for the EMA side rule (TREND_EMA_ONLY only)
//--- v1.13, 17 Sep 2026: THE INVALIDATION LINE. The EMA that picks the side
//    is also the level that invalidates it. That is the coach's own rule
//    ("bearish while under the Daily EMA9"); he applies it at the daily
//    close and never sells above it, and until now the EA applied it
//    nowhere. On 17 Sep the daily close put the side at SELL under a line
//    of 4,329.76 and gold rallied 90.00 through it. The EA kept resting
//    sell limits at zones ABOVE the line and two of them filled (19:09 at
//    4,333.84, 19:53 at 4,338.68), both stopped: -203.60 of a -306.00 day.
//    His map that morning had no resistance above 4,317.
//    RULE: on a SELL day no sell limit or rejection entry may sit above the
//    line; on a BUY day none below it. While price is beyond the line every
//    zone on the trade side is beyond it too, so the EA is paused with no
//    extra state, and it resumes by itself when price comes back.
//    It does NOT flip the side. The day before (16 Sep) the same signal
//    fired: price crossed the line at 4,344, ran to 4,380, and the daily
//    candle closed at 4,271.57. An intraday flip buys the top of that day.
//    The flip stays where the coach keeps it, at the daily close.
//    Checked against every fill on the Daily rule, 14-17 Sep: removes the
//    two above, blocks no winner. Tester result in the README.
//    Only meaningful with TREND_EMA_ONLY; ignored in the other modes.
input bool   UseInvalidationLine = true; // No entries beyond the EMA that picked the side (sells only below it, buys only above)
...
//--- v1.7, 10 Sep 2026: 15M swing points, so the EA sees the same intraday
//    levels the coach marks on his 15M chart (his R1 4,420 was the 09:00
//    spike high, his S2 4,391 the 06:00 swing low; neither is an H1 or 4H
//    swing, so v1.6 had no zone at either). Pivots are recency-weighted
//    (0.6 to 1.0 of the base weight), so at 2.5 the newest 15M swings clear
//    ZoneMinScore=2.0 on their own and the older half only count with
//    something else stacked on them. Cost: more zones, nearer limits, and
//    tighter stops - the kind of trade that lost on real ticks.
input bool   UseEntryPivots     = true;  // Swing points on EntryTF (15M) as levels
input int    EntryPivotLookback = 192;   // EntryTF bars scanned (192 x 15M = 2 days)
input double EntryPivotWeight   = 2.5;   // Weight of a 15M swing point (newest full, oldest 60%)
...
input group "=== ZONES ==="
//--- v1.7.2, 10 Sep 2026: FIXED merge width. The ATR-based width tracks
//    volatility, which sounds right and is not: on a day with an H1 ATR of
//    15.00 it merged anything within 5.23 and grew bands to 10.00, so the
//    coach's S2 4391 and S3 4375-4386 became one zone and his S1 4400-4405
//    was swallowed together with the MA50. He draws lines and 5.00 bands
//    whatever the ATR is. 250 pips keeps a band under 5.00. Set 0 to go
//    back to the ATR multiple.
input double ZoneMergePips      = 250;   // Merge points this close (0 = use ATR multiple below)
input double ZoneMergeAtrMult   = 0.35;  // ...as a multiple of ATR(14) on PivotTF, when the above is 0
...
//--- v1.7.2: 3000 hid the coach's R3 4442 from a market at 4400.
input double MaxZoneDistancePips= 4500;  // Ignore zones farther than this from the market
...
input group "=== ENTRY ==="
input ENUM_ENTRY_MODE EntryMode = ENTRY_LIMIT; // How a zone is entered (when UsePerTierEntry is false)
//--- v1.9, 14 Sep 2026: the entry style can differ PER TIER. Operator's
//    idea, and the reasoning is sound: tier 1 is the nearest zone, the one
//    price is already travelling towards, so it is the one most often
//    sliced straight through - four of the first six live losses were
//    exactly that. Tiers 2 and 3 are only reached on an extended move,
//    which is likelier to be genuine exhaustion, and a resting limit gets
//    a better price there than waiting for a close.
//    Set UsePerTierEntry=false to go back to one style for all tiers.
input bool   UsePerTierEntry    = true;  // Use the three per-tier styles below instead of EntryMode
input ENUM_ENTRY_MODE Tier1Entry = ENTRY_REJECTION; // S1 / R1 style
input ENUM_ENTRY_MODE Tier2Entry = ENTRY_LIMIT;     // S2 / R2 style
input ENUM_ENTRY_MODE Tier3Entry = ENTRY_LIMIT;     // S3 / R3 style
//--- v1.11.4, 15 Sep 2026: OFF, on live evidence that contradicts every
//    backtest. Nine resolved trades since the EA went live on 10 Sep:
//
//      tier 1   0 wins  5 losses   -546.60
//      tier 2   4 wins  0 losses   +773.70
//
//    Tier 1 lost three times as a resting LIMIT (10-11 Sep) and twice as a
//    REJECTION (14-15 Sep), so the entry style is not the difference - the
//    zone is. That matters, because it rules out the v1.9 per-tier change as
//    the cause.
//    Against this: every minute-bar test made tier 1 the profitable bucket
//    and tier 2 the marginal one - +952 against -820 in 2026, +2912 against
//    +886 in 2025 - and tier 1 supplies roughly 90% of all fills, so this
//    cuts the trade count hard. A 0-for-5 run at a 40% win rate happens
//    about one time in thirteen; 4-for-4 about one in forty. The split looks
//    real and the sample cannot prove it.
//    Set true to restore. If tiers 2 and 3 go cold on their own, this is the
//    first thing to put back.
input bool   TradeTier1         = false; // Trade S1 / R1 (aggressive)
input bool   TradeTier2         = true;  // Trade S2 / R2 (default)
//--- v1.11, 14 Sep 2026: ON at the operator request. It was switched off
//    on 10 Sep as a judgment call before any testing, then measured on the
//    original build and left off: -1036 against -845 for the same build
//    without it. That was six versions ago and the EA has changed in almost
//    every part since, so the old number should not be treated as current.
//    NOTE the interaction: MaxConcurrentPositions is 2 and resting limits
//    count against it, so with tiers 1 and 2 active there is often no slot
//    left for a third.
//    Measured on THIS build, minute bars, per-tier entries, cap 3 fills/day:
//
//                        2026 Jan-Sep            2025
//      tier 3 off       -517  PF 0.96  DD 12.0%  +3463  PF 1.14  DD 9.7%
//      tier 3 on        -73   PF 0.99  DD 12.4%  +3892  PF 1.16  DD 8.0%
//      + MaxPos 3       -368  PF 0.97  DD 12.8%  +4520  PF 1.17  DD 8.8%
//
//    Better in BOTH years, which the 10 Sep measurement was not. The slot
//    squeeze turns out to be the mechanism rather than an obstacle: tier 3
//    takes ~38 trades a year and displaces far more tier-2 trades than that,
//    and tier 2 is the losing bucket (-1169 -> -820 in 2026). Raising
//    MaxConcurrentPositions to 3 removes that squeeze, which is why it wins
//    2025 by 628 and gives back 295 in 2026. Left at 2 deliberately.
input bool   TradeTier3         = true;  // Trade S3 / R3 (conservative)
...
//--- v1.11.7, 15 Sep 2026: 300 -> 500. A zone built from three pivots
//    that happened to land within 0.63 of each other (4310.64-4311.27)
//    produced a 3.60 stop at 0.29 lots on a quiet-ATR morning, and price
//    took it in 38 minutes for -117.80. A narrow zone is not a safer level,
//    it is just a narrow cluster, and the stop was measuring cluster width
//    rather than how far a bounce needs to breathe. 5.00 is the floor now.
//    What changes with it, since the 2R floor scales off the stop: the
//    smallest target becomes 10.00, and the lot on a thin zone roughly
//    halves. Untested; the operator asked for it directly.
input double MinSLPips          = 500;   // Widen any stop tighter than this
...
//--- v1.11.5, 15 Sep 2026: 3 -> 5 at the operator request. The measured
//    cost is recorded here rather than argued, because the operator asked
//    for it after seeing the first sweep and then again with tier 1 off.
//
//    Sweep on 14 Sep, tier 1 ON, minute bars, lot cap lifted:
//      cap 2   -158 / +3307   cap 3   -517 / +3463
//      cap 4   -109 / +2614   cap 5  -1435 / +2101   cap 8  -2095 / +2844
//
//    Re-run on 15 Sep with tier 1 OFF, which is the shipping configuration:
//      cap 3  -2932 PF 0.74 DD 31.7%  /  +36  PF 1.00 DD 10.0%
//      cap 5  -2019 PF 0.88 DD 25.4%  /  -1204 PF 0.91 DD 17.5%
//
//    Note what that second table says about the FIRST decision, not this
//    one: with tier 1 gone the whole system is at or below breakeven at
//    every cap. Cap 5 is better than cap 3 in 2026 and much worse in 2025.
//    There is no good setting here; the damage was done upstream when the
//    profitable bucket was switched off.
//--- v1.11.6, same day: UNLIMITED at the operator request. Measured with
//    tier 1 on, unlimited was -1616 / +2526 against -517 / +3463 at cap 3.
//    With tier 1 off it was not measured; the operator asked to stop testing.
//    WHAT IS LEFT HOLDING THE LINE: with no fill cap, the only remaining
//    brakes on a bad day are MaxConcurrentPositions (2) and
//    MaxDailyLossPercent (5.0). The fill cap was the one that stopped a
//    grinding day early. On this account 5% is roughly ten full stops.
input int    MaxTradesPerDay    = 0;     // Max fills per trading day (0 = unlimited)
input bool   BlockOpposingEntries = true; // Never hold a BUY and a SELL at once (set false for TREND_NONE)
input double MaxDailyLossPercent= 5.0;   // Halt for the day at this realised loss % (0 = off)
//--- v1.11.3, 15 Sep 2026: MIDNIGHT MANILA, not 17:00 ET. The default from
//    v1.0 was 17:00 ET, the forex day boundary. At 03:51 local the operator
//    saw DAY CAP 3/3 still holding from the previous session and expected a
//    fresh day, then asked for midnight in their own zone.
//    Manila is GMT+8 and ETOffsetHours is -4, so midnight there is NOON ET.
//    This input is in ET, hence 12 rather than 0 - v1.11.2 shipped 0 for a
//    few minutes, which is midnight ET and noon in Manila, exactly backwards.
//    Counters now reset while the operator is asleep rather than mid-session.
//    Note this is the same boundary TrendEMA uses.
//    WATCH THE DST EDGE: ETOffsetHours is a manual input. When New York
//    leaves daylight time it becomes -5, and until it is changed the rollover
//    drifts to 01:00 Manila.
...
//--- v1.12, 15 Sep 2026: NY-session blackout, in EASTERN time so it follows
//    ETOffsetHours and does not need re-typing when New York changes clocks.
//    Operator: "8am to 6pm EST, there should be no trades, NY session is too
//    volatile." That is 20:00-06:00 in Manila, so the EA now trades only
//    while the operator is awake to see it.
//    PRIOR EVIDENCE, and it points the other way: a near-identical window was
//    tested on 10 Sep as a GMT trade-window (22:00-13:00 GMT, i.e. 18:00-09:00
//    ET allowed) and was REJECTED for failing out of sample - 2026 +1263
//    PF 1.28 against a +530 baseline, but 2025 only +236 PF 1.04 against
//    +1053. It looked like an edge in the year it was fitted to and nearly
//    vanished in the other one. Shipped anyway at the operator request; the
//    reasoning this time is volatility tolerance rather than backtest return.
//    Empty string disables it. It cancels resting limits while active, and it
//    does NOT close an open position.
input string SessionBlackoutET  = "08:00-18:00"; // ET hours to sit out (empty = none)
...
int      hEmaSide = INVALID_HANDLE;  // v1.8: EMA on TrendEMATF, drives TREND_EMA_ONLY
...
//--- v1.8: closed-bar close and EMA on TrendEMATF. Kept separate from the
//    TrendTF pair above so the panel can show both.
double   gSideClose = 0, gSideEma = 0;
datetime gBeyondSince = 0;     // v1.13: when price first went beyond the invalidation line (0 = it has not)
datetime gBeyondLogBar = 0;    // v1.13.1: entry bar of the last INVALIDATION log line - one line per bar at most
...
double   gAllNet = 0;
//--- v1.7.3: gross win / loss so the breakeven win rate can come from what
//    actually happened (average loss / (average win + average loss)) rather
//    than from the nominal 2R floor. Falls back to the nominal until there
//    is at least one of each.
double   gAllGrossWin = 0, gAllGrossLoss = 0;
...
int      gObjCountAfterPanel = -1;   // v1.13.3: chart objects the last time the panel was on top
...
int OnInit()
  {
   ...
   hEmaSide= iMA(_Symbol, TrendEMATF, TrendEMAPeriod, 0, MODE_EMA, PRICE_CLOSE);
   ...
   if(hIchiT == INVALID_HANDLE || hIchiE == INVALID_HANDLE || hMaT == INVALID_HANDLE ||
      ...
      hAtrE == INVALID_HANDLE || hEmaSide == INVALID_HANDLE)
   ...
   RefreshTrend(true);
   //--- v1.7.1: the entry-TF read (ATRs, 15M cloud and MA, D1 MAs) used to
   //    happen on the first tick, AFTER this first zone build. So the build
   //    ran with gAtrP = 0, MergeWidth fell back to 150 pips, and the levels
   //    came out as 1-2 dollar slivers: "136 points -> 65 zones (merge 150p)"
   //    on the 16:32 attach, six of them stacked between 4390 and 4413 where
   //    the coach had two. The first limits of every attach today rested on
   //    that set until the next 15M close rebuilt it properly. Read first.
   RefreshEntry(true);
   RebuildZones(true);

   //--- v1.9.1: "Trend=%s on %s" printed "D1 close vs EMA9 on PERIOD_H4",
   //    because TrendModeName() names its OWN timeframe in TREND_EMA_ONLY
   //    while the second slot still printed TrendTF. The rule was reading D1
   //    correctly; only the text was wrong. TrendTF supplies the cloud and
   //    MA50 in that mode, nothing else, so it is no longer printed as if it
   //    were the side.
   PrintFormat("[SRL] Initialised. Symbol=%s Digits=%d Pip=%.5f  Side=%s  Entry=%s  "
               "Pivots=%s/%s  Tiers=%s%s%s  SL=%s buffer %.0fp/%.1fxATR [%0.f..%.0f]  TP=%s minRR %.2f  "
               "Risk=%.2f%% MaxPos=%d MaxDay=%d",
               _Symbol, gDigits, gPip,
               (TrendMode == TREND_EMA_ONLY)
                  ? TrendModeName()
                  : TrendModeName() + " on " + TfName(TrendTF),
               EntryModeText(),
               EnumToString(PivotTF), UseEntryPivots
                  ? EnumToString(MajorPivotTF) + "/" + EnumToString(EntryTF) : EnumToString(MajorPivotTF),
   ...
   PrintFormat("[SRL] Invalidation line: %s",
               (UseInvalidationLine && TrendMode == TREND_EMA_ONLY)
                  ? StringFormat("ON - no sells above / no buys below the %s EMA%d that picked the side (now %.2f)",
                                 TfName(TrendEMATF), TrendEMAPeriod, gSideEma)
                  : (UseInvalidationLine ? "OFF (needs TREND_EMA_ONLY)" : "OFF"));
   PrintFormat("[SRL] NY blackout: %s (Eastern, ET offset %+d from GMT)",
               StringLen(SessionBlackoutET) > 0 ? SessionBlackoutET : "OFF", ETOffsetHours);
...
void OnDeinit(const int reason)
  {
   ...
   int hs[11]; hs[0]=hIchiT; hs[1]=hIchiE; hs[2]=hMaT; hs[3]=hEmaT; hs[4]=hMa200T;
   hs[5]=hMaE; hs[6]=hMaD1; hs[7]=hEmaD1; hs[8]=hAtrP; hs[9]=hAtrE; hs[10]=hEmaSide;
   for(int i = 0; i < 11; i++) if(hs[i] != INVALID_HANDLE) IndicatorRelease(hs[i]);
  }
...
void OnTick()
  {
   ...
   if(gTrendDir != gTrendPrev)
     {
      if(gTrendPrev != 0 || gTrendDir != 0)
         PrintFormat("[SRL] Side changed: %s -> %s  [%s]  (%s)", DirName(gTrendPrev),
                     DirName(gTrendDir),
                     TrendMode == TREND_EMA_ONLY
                        ? StringFormat("%s close %.2f vs EMA%d %.2f", TfName(TrendEMATF),
                                       gSideClose, TrendEMAPeriod, gSideEma)
                        : TrendModeName(),
                     gTrendText);
      ...
      gBeyondSince = 0;                              // v1.13: a new side starts clean
     }

   //--- v1.13: invalidation-line state. The rule itself needs none (see
   //    UseInvalidationLine); this records when price first went beyond the
   //    line so the panel can say "since hh:mm" and the log shows how often
   //    a side is invalidated intraday and for how long.
   if(LineActive() && (gTrendDir == 1 || gTrendDir == -1))
     {
      double px = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      bool beyondNow = BeyondLine(gTrendDir, px);
      //--- v1.13.1: the state still updates on every tick (the panel and the
      //    rule need it), but the LOG gets at most one line per entry bar.
      //    Price chatters across the line: the 2026 tester run printed 1,512
      //    crossing lines, about eight a day, most of them seconds apart.
      if(beyondNow && gBeyondSince == 0)
        {
         gBeyondSince = TimeCurrent();
         if(gBeyondLogBar != gLastEntryBar)
           {
            gBeyondLogBar = gLastEntryBar;
            PrintFormat("[SRL] INVALIDATION: price %.2f is %s the %s EMA%d line %.2f - %s side paused, no %s beyond the line",
                        px, gTrendDir < 0 ? "above" : "below", TfName(TrendEMATF), TrendEMAPeriod, gSideEma,
                        DirName(gTrendDir), gTrendDir < 0 ? "sells" : "buys");
           }
        }
      else if(!beyondNow && gBeyondSince != 0)
        {
         if(gBeyondLogBar != gLastEntryBar)
           {
            gBeyondLogBar = gLastEntryBar;
            PrintFormat("[SRL] INVALIDATION cleared: price %.2f back %s the line %.2f after %d min - %s side resumes",
                        px, gTrendDir < 0 ? "below" : "above", gSideEma,
                        (int)((TimeCurrent() - gBeyondSince) / 60), DirName(gTrendDir));
           }
         gBeyondSince = 0;
        }
     }
   ...
   //--- v1.13.3: zones BEFORE the panel. They were drawn after it, so on
   //    every entry bar the six freshly created rectangles and their text
   //    sat above the panel until something else forced a rebuild.
   if(!gHeadless && (newEntryBar || gLastPanelMs == 0 || (nowMs - gLastPanelMs) > 300))
     { gPhase = "DRAW"; if(newEntryBar || gLastPanelMs == 0) DrawZones(); DrawPanel(); gLastPanelMs = nowMs; }
  }
...
//--- v1.13: the invalidation line is gSideEma, the closed-bar EMA on
//    TrendEMATF that picked the side. A price is "beyond" it when it sits
//    on the wrong side for the trade: above it for a sell, below for a buy.
//    Exactly on the line is allowed - the EA's own R-zone built from
//    [MA50 D1, EMA9 D1] has the line as its top edge and sells at its
//    bottom edge, which is selling AT resistance, not above it.
bool LineActive() { return(UseInvalidationLine && TrendMode == TREND_EMA_ONLY && gSideEma > 0.0); }
bool BeyondLine(const int dir, const double price)
  {
   if(!LineActive()) return(false);
   return(dir < 0 ? price > gSideEma : (dir > 0 ? price < gSideEma : false));
  }

//--- v1.9: entry style for tier index t (0-based)
ENUM_ENTRY_MODE TierMode(const int t)
  {
   if(!UsePerTierEntry) return(EntryMode);
   if(t == 0) return(Tier1Entry);
   if(t == 1) return(Tier2Entry);
   return(Tier3Entry);
  }

string TierModeLetter(const int t) { return(TierMode(t) == ENTRY_LIMIT ? "L" : "R"); }

string EntryModeText()
  {
   if(!UsePerTierEntry) return(EntryMode == ENTRY_LIMIT ? "LIMIT" : "REJECTION");
   return(StringFormat("per-tier %s/%s/%s (L=limit R=rejection)",
                       TierModeLetter(0), TierModeLetter(1), TierModeLetter(2)));
  }
   ...
   switch(TrendMode)
     {
      ...
      case TREND_EMA_ONLY:     return(StringFormat("%s close vs EMA%d", TfName(TrendEMATF), TrendEMAPeriod));
...
bool RefreshTrend(const bool force)
  {
   ...
   if(CopyClose(_Symbol, TrendTF, 1, 1, c) == 1 &&
      ...
      CopyOne(hMaT, 0, 1, gTMa) && CopyOne(hEmaT, 0, 1, gTEma))
     {
      ...
      //--- v1.8: the side pair. Falls back to the TrendTF pair if the read
      //    fails, so a missing daily bar cannot silently park the EA on
      //    dir 0 and stop it trading for a whole day.
      double sc[];
      if(CopyClose(_Symbol, TrendEMATF, 1, 1, sc) == 1 && CopyOne(hEmaSide, 0, 1, gSideEma))
         gSideClose = sc[0];
      else
        { gSideClose = gTClose; gSideEma = gTEma; }
      ...
      switch(TrendMode)
        {
         ...
         case TREND_EMA_ONLY:     dir = (gSideClose > gSideEma) ? 1 : ((gSideClose < gSideEma) ? -1 : 0); break;
...
double MergeWidth()
  {
   ...
   if(gAtrP <= 0.0) CopyOne(hAtrP, 0, 1, gAtrP);     // v1.7.1: never build on a zero ATR
...
void RebuildZones(const bool force)
  {
   ...
   if(UseEntryPivots) CollectPivots(EntryTF, EntryPivotLookback, EntryPivotWeight, "piv " + eN);
...
//--- v1.12: one parser for both window lists. "HH:MM-HH:MM,HH:MM-HH:MM".
//    nowM is minutes past midnight in whatever zone the caller is working in.
bool InWindowSpec(const string spec, const int nowM)
  {
...
//--- Eastern wall-clock, same convention the day rollover uses, so both
//    follow ETOffsetHours together when New York changes clocks.
datetime NowET() { return(TimeGMT() + (datetime)(ETOffsetHours * 3600)); }

int NowETMinutes()
  {
   MqlDateTime e; TimeToStruct(NowET(), e);
   return(e.hour * 60 + e.min);
  }
...
bool InSessionWindow()
  {
   ...
   return(InWindowSpec(SessionWindowsGMT, g.hour * 60 + g.min));
  }

//--- v1.12: the NY blackout. Separate from SessionWindowsGMT on purpose -
//    "trade only during these hours" and "never trade during these hours" are
//    different questions, and folding them into one parser was what silently
//    disabled the clock filter in TrendEMA when its news gate was switched off.
bool InSessionBlackout()
  {
   if(StringLen(SessionBlackoutET) == 0) return(false);
   return(InWindowSpec(SessionBlackoutET, NowETMinutes()));
  }
...
void EvaluateAndAct(const bool newEntryBar)
  {
   ...
   if(InSessionBlackout())
     {
      MqlDateTime e; TimeToStruct(NowET(), e);
      SetStatus("BLACKOUT", StringFormat("%s ET  (now %02d:%02d ET)", SessionBlackoutET, e.hour, e.min), clrOrange);
      CancelAllPendings("NY session blackout");
      return;
     }
   if(IsFridayCutoff())
   ...
   //--- v1.9: both paths run unconditionally and each skips the tiers that
   //    are not its style. ManageLimits is called even with every tier on
   //    rejection, because it also owns the cancel sweep - without it a
   //    limit left over from a style switch would rest forever.
   ...
   for(int t = 0; t < 3; t++)
     {
      ...
      if(!enabled) continue;
      //--- v1.9: a rejection tier rests nothing. Its zone is left unclaimed
      //    on purpose, so the sweep at the end of this function cancels any
      //    limit it held from before the style was switched.
      if(TierMode(t) != ENTRY_LIMIT) continue;
      ...
      //--- v1.13: no limit beyond the invalidation line. A resting one that
      //    is now beyond it (the line moved at the daily close) is cancelled.
      if(BeyondLine(dir, entry))
        { if(ex >= 0) { CancelPendingRec(ex, "beyond the invalidation line"); ArrayRemove(claimed, ex, 1); } state += tname + " beyond-line  "; continue; }
   ...
   for(int t = 0; t < 3; t++)
     {
      bool enabled = (t == 0 && TradeTier1) || (t == 1 && TradeTier2) || (t == 2 && TradeTier3);
      if(!enabled) continue;
      if(TierMode(t) != ENTRY_REJECTION) continue;   // v1.9
      int zi = (dir > 0) ? gSup[t] : gRes[t];
      if(zi < 0) continue;
      Zone z = gZones[zi];
      string tname = StringFormat("%s%d", dir > 0 ? "S" : "R", t + 1);
      ...
      //--- v1.11.1: count resting limits too. This check used to count only
      //    OPEN positions while ManageLimits counted positions PLUS pendings,
      //    so with tiers 2 and 3 both resting a limit - which is the normal
      //    state since v1.11 turned tier 3 on - a tier-1 rejection fired
      //    anyway. Both limits could then fill and the account would hold
      //    three positions against a cap of two: 1.5% of equity at risk on
      //    one correlated move instead of 1%. The cap now means what it says
      //    on both paths.
      //    COST, measured, minute bars, tier 3 on, cap 2:
      //                        2026 Jan-Sep            2025
      //      cap counted loose -73   PF 0.99  DD 12.4%  +3892  PF 1.16  DD 8.0%
      //      cap enforced      -971  PF 0.93  DD 17.0%  +2565  PF 1.12  DD 11.4%
      //    About -2200 across the two years. Tier 1 is the winning bucket and
      //    enforcing the cap blocks 40 of its 170 entries in 2026 and 88 of
      //    541 in 2025. The operator chose it anyway, on risk grounds, with
      //    these numbers in front of them: a cap of 2 that quietly reaches 3
      //    is 1.5% of equity on one correlated move. Set the check back to
      //    CountPositions() alone to recover the money and the exposure.
      int open = CountPositions() + CountOurPendings();
      ...
      if(BeyondLine(dir, entry)) { state += tname + " beyond-line  "; continue; }   // v1.13
      double sl = 0, tp = 0;
...
void RefreshAllTimeStats()
  {
   ...
   gAllWins = 0; gAllLosses = 0; gAllNet = 0.0; gAllGrossWin = 0.0; gAllGrossLoss = 0.0;
   int total = HistoryDealsTotal();
   for(int i = 0; i < total; i++)
     {
      ulong t = HistoryDealGetTicket(i); if(t == 0) continue;
      ...
      double net = HistoryDealGetDouble(t, DEAL_PROFIT) + HistoryDealGetDouble(t, DEAL_SWAP) + HistoryDealGetDouble(t, DEAL_COMMISSION);
      gAllNet += net;
      if(net > 0.0) { gAllWins++; gAllGrossWin += net; }
      else if(net < 0.0) { gAllLosses++; gAllGrossLoss -= net; }
     }
  }
...
//--- v1.13.3: MT5 renders chart objects in the order they were CREATED.
//    Nothing in the API re-orders them: OBJPROP_ZORDER only decides which
//    object receives a mouse click, and OBJPROP_BACK only picks the
//    background layer, so a foreground object made later always paints over
//    a foreground object made earlier. The panel is created once at attach
//    and then only has its text updated, so every object drawn afterwards -
//    the zone rectangles on each entry bar, an indicator's arrows, a line
//    the operator draws - ends up on top of it. On 18 Sep the panel was
//    unreadable behind a drawing, with R2 and the LIMITS row covered.
//    Deleting the panel objects and letting DrawPanel build them again puts
//    them last in the list, which is the only way to raise them. Doing that
//    on every refresh would flicker, so it is driven by the object count:
//    when anything has been added or removed since the panel was last
//    rebuilt, rebuild it. The count is re-read AFTER the rebuild, so the
//    panel's own delete-and-recreate does not trigger the next one.
void RaisePanel()
  {
   if(!ShowPanel || gHeadless) return;
   ObjectDelete(0, PFX + "BG");
   for(int r = 0; r < gPanelRowsDrawn; r++)
     {
      ObjectDelete(0, PFX + "PL" + IntegerToString(r));
...
void DrawPanel()
  {
   if(!ShowPanel || gHeadless) return;
   if(gObjCountAfterPanel >= 0 && ObjectsTotal(0) != gObjCountAfterPanel) RaisePanel();
   ...
   PanelRow(row++, "SR LEVELS EA v1.13.3", real ? "REAL MONEY" : "demo", clrGold, real ? clrRed : clrSilver);
   ...
   PanelRow(row++, "ENTRY", EntryModeText(), PanelTextColor, clrSilver);
   PanelRow(row++, "SIDE", StringFormat("%s  (%s)", DirName(gTrendDir), TrendModeName()), PanelTextColor,
            gTrendDir > 0 ? clrLime : (gTrendDir < 0 ? clrTomato : clrSilver));
   if(TrendMode == TREND_EMA_ONLY)
      PanelRow(row++, "SIDE FROM", StringFormat("%s close %.2f vs EMA%d %.2f", TfName(TrendEMATF),
               gSideClose, TrendEMAPeriod, gSideEma), PanelTextColor, clrSilver);
   //--- v1.13: the line, where price is against it, and what that means
   if(LineActive() && (gTrendDir == 1 || gTrendDir == -1))
     {
      double px = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      bool beyond = BeyondLine(gTrendDir, px);
      //--- v1.13.2: PanelLabel cuts every value at 64 characters. The v1.13
      //    row built 96 and the live panel showed "...INVALIDATED sinc...",
      //    losing the time and the whole point of the row. The price itself
      //    is on the MARKET row two lines down, so ABOVE/BELOW carries it
      //    and the string fits with room to spare. Keep any edit to this row
      //    under 64 characters at the longest state, or widen the cap in
      //    PanelLabel - do not just add words here.
      //      "EMA9 D1 4329.76   price ABOVE   SELLS PAUSED since 23:46" = 56
      PanelRow(row++, "INVALIDATION", StringFormat("EMA%d %s %.2f   price %s   %s",
               TrendEMAPeriod, TfName(TrendEMATF), gSideEma,
               px > gSideEma ? "ABOVE" : (px < gSideEma ? "BELOW" : "AT"),
               beyond ? StringFormat("%s PAUSED since %s", gTrendDir < 0 ? "SELLS" : "BUYS",
                                     TimeToString(gBeyondSince, TIME_MINUTES))
                      : StringFormat("%s allowed", gTrendDir < 0 ? "sells" : "buys")),
               PanelTextColor, beyond ? clrOrange : clrSilver);
     }
   else if(TrendMode == TREND_EMA_ONLY)
      PanelRow(row++, "INVALIDATION", UseInvalidationLine ? "waiting for a side" : "OFF", PanelTextColor, clrGray);
   PanelRow(row++, TfName(TrendTF) + " VALUES", StringFormat("close %.2f  EMA%d %.2f  MA50 %.2f  cloud %.2f/%.2f", gTClose,
            TrendEMAPeriod, gTEma, gTMa, MathMin(gTSenA, gTSenB), MathMax(gTSenA, gTSenB)), PanelTextColor, clrSilver);
   ...
   //--- v1.9.2: P/L as a percent of capital, the way the TrendEMA panel
   //    shows it. TODAY is against the balance at the 17:00 ET rollover,
   //    which is the same denominator MaxDailyLossPercent halts on, so the
   //    number here can be read straight against that 5.0 limit.
   PanelRow(row++, "TODAY", StringFormat("%d/%s fills  %dW %dL  P/L %+.2f%s", gDayTrades,
            MaxTradesPerDay > 0 ? IntegerToString(MaxTradesPerDay) : "-", gDayWins, gDayLosses,
            gDayPL,
            gDayStartBalance > 0.0
               ? StringFormat(" (%+.2f%% of %.0f)", 100.0 * gDayPL / gDayStartBalance, gDayStartBalance)
               : ""),
            PanelTextColor, gDayPL >= 0 ? clrLime : clrTomato);
   double eqNow = AccountInfoDouble(ACCOUNT_EQUITY);
   PanelRow(row++, "ALL TIME", StringFormat("%dW %dL  net %+.2f%s", gAllWins, gAllLosses, gAllNet,
            eqNow > 0.0 ? StringFormat(" (%+.2f%% of %.0f)", 100.0 * gAllNet / eqNow, eqNow) : ""),
            PanelTextColor, clrSilver);
   //--- v1.7.3: the rates side by side, and the rate they have to beat.
   //    "needs" is the realised breakeven once there is a win and a loss on
   //    record (average loss / (average win + average loss)); before that it
   //    is the nominal one for the reward floor, 1 / (1 + MinRR).
   int dN = gDayWins + gDayLosses, aN = gAllWins + gAllLosses;
   double needs = (gAllWins > 0 && gAllLosses > 0)
                  ? 100.0 * (gAllGrossLoss / gAllLosses) / ((gAllGrossWin / gAllWins) + (gAllGrossLoss / gAllLosses))
                  : 100.0 / (1.0 + MinRR);
   double aRate = aN > 0 ? 100.0 * gAllWins / aN : 0.0;
   PanelRow(row++, "WIN RATE", StringFormat("today %s   overall %s   needs %.0f%%",
            dN > 0 ? StringFormat("%.0f%% (%d/%d)", 100.0 * gDayWins / dN, gDayWins, dN) : "- (0/0)",
            aN > 0 ? StringFormat("%.0f%% (%d/%d)", aRate, gAllWins, aN) : "- (0/0)", needs),
            PanelTextColor, (aN > 0 && aRate >= needs) ? clrLime : (aN > 0 ? clrTomato : clrSilver));
   ...
   gObjCountAfterPanel = ObjectsTotal(0);   // v1.13.3: the baseline for the next raise
   ChartRedraw();
  }
