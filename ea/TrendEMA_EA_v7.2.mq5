...
#property version   "7.20"   // file is TrendEMA_EA_v7.2 - bump both together
...
input int    PanelRefreshSeconds = 5;    // Redraw the panel this often even with no ticks
...
//--- last tick arrival, so the panel can show its own staleness
datetime gLastTickTime = 0;
...
void DrawPanel()
  {
   ...
   bool isReal = ((ENUM_ACCOUNT_TRADE_MODE)AccountInfoInteger(ACCOUNT_TRADE_MODE)
                  == ACCOUNT_TRADE_MODE_REAL);
   int tickAge = (gLastTickTime > 0) ? (int)(TimeCurrent() - gLastTickTime) : 999;
   PanelRow(r++, StringFormat("TREND EA  -  %s", _Symbol),
            StringFormat("%s   tick %ds", isReal ? "REAL" : "demo", tickAge),
            clrGold, tickAge > 60 ? clrRed : (isReal ? clrOrange : clrLime));
