...
#property version   "7.19"   // file is TrendEMA_EA_v7.19 - bump both together
...
double CounterTargetPips(const int dir, const double entry,
                         double &barrierOut, double &roomOut)
  {
   ...
   if(barrierOut <= 0.0)
      return(TakeProfitCounterPips);      // clear air - keep the fixed target

   roomOut = MathAbs(entry - barrierOut) / gPip - CounterBarrierBuffer;
   if(roomOut < 0.0)
      roomOut = 0.0;
   return(MathMin(TakeProfitCounterPips, roomOut));
  }
...
double ConfigSignature()
  {
   string sig = StringFormat(
      "%s|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%.1f|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%d|%.1f|%.1f|%.1f|%d|"
