...
#property version   "1.100"  // file is SRLevels_EA_v1.10 - bump both together
...
input bool   UseFib             = true;  // Fib retracement of the last TrendTF swing
input int    FibSwingBars       = 60;    // TrendTF bars scanned for the swing high / low
input double FibWeight          = 1.0;   // Weight of a fib level
//--- v1.10, 14 Sep 2026: four sources the coach uses that the EA had no way
//    of seeing. From the gap list in ea/COACH_LOG.md, after the operator
//    compared his charts against the panel three days running.
//      - Daily swings: his 4,311 neckline and 4,292 support are D1 pivots.
//      - Daily cloud: he named the D1 MA50 + Kumo confluence at 4,268 as a
//        bounce level. The EA had that MA50 but not that kumo.
//      - Daily fib: his 0.618 at 4,230.98 and 0.786 at 4,104.30 come off a
//        Daily swing. The EA only ever drew fib on TrendTF.
//      - Round numbers: 4,300 and 4,400, named in two consecutive posts as
//        psychological support and resistance.
//    Weighted to CONTRIBUTE to a zone rather than to stand alone, except the
//    Daily swing, which is as real a level as a 4H one.
//
//--- ALL FOUR DEFAULT TO FALSE. They were built, tested the same afternoon,
//    and every one of them lost money in BOTH years. Minute bars, per-tier
//    entries, cap 3, MaxLotSizeCap=0, against the v1.9.1 baseline:
//
//                       2026 Jan-Sep              2025
//      baseline        -517  PF 0.96  DD 12.0%   +3463  PF 1.14  DD  9.7%
//      all four        -1524 PF 0.88  DD 18.9%   +2222  PF 1.10  DD  9.6%
//      D1 pivots only  -1212 PF 0.91              +2316  PF 1.10
//      round only      -1134 PF 0.91              +2903  PF 1.12
//      D1 fib only     -693  PF 0.95              +3226  PF 1.13
//      D1 cloud only   -517  (no change)          +3140  PF 1.13
//
//    The trade COUNT barely moves (482 -> 486 in 2026), so these are not
//    adding trades; they are shifting the boundaries and the ranking of the
//    zones that were already being traded, and shifting them worse. Almost
//    all the damage lands in the tier-2 bucket: -1169 -> -1914 in 2026.
//
//    Worth stating plainly, because it settles a question the operator has
//    been asking for days: the gap between this EA and the coach is NOT the
//    level list. He has more sources and the EA now can too, and copying
//    them makes it worse. Whatever he does that works is in the judgment and
//    the geometry, not in what he draws.
input bool   UseDailyPivots     = false; // Swing points on D1 as levels (tested: loses)
input int    DailyPivotLookback = 120;   // D1 bars scanned for swings
input double DailyPivotWeight   = 2.5;   // Weight of a D1 swing point
input bool   UseCloud_D1        = false; // D1 cloud edges as levels (tested: loses)
input bool   UseFibD1           = false; // Fib retracement of the last D1 swing (tested: loses)
input int    FibD1SwingBars     = 90;    // D1 bars scanned for that swing
input double FibD1Weight        = 1.5;   // Weight of a D1 fib level
input bool   UseRoundNumbers    = false; // Psychological levels (tested: loses)
input double RoundNumberStep    = 5000;  // ...every this many pips (5000 = 50.00)
input double RoundNumberWeight  = 1.0;   // Weight of a round number
...
int      hIchiD1  = INVALID_HANDLE;  // v1.10: D1 cloud, levels only - it gates nothing
...
double   gDSenA = 0, gDSenB = 0;      // v1.10: D1 cloud edges
...
int OnInit()
  {
   ...
   hIchiD1 = iIchimoku(_Symbol, PERIOD_D1, IchiTenkan, IchiKijun, IchiSenkou);
   ...
   if(hIchiT == INVALID_HANDLE || hIchiE == INVALID_HANDLE || hMaT == INVALID_HANDLE ||
      ...
      hAtrE == INVALID_HANDLE || hEmaSide == INVALID_HANDLE || hIchiD1 == INVALID_HANDLE)
   ...
   PrintFormat("[SRL] Sources: MA50 15M %s | MA50 4H %s | MA50 D1 %s | EMA9 4H %s | EMA9 D1 %s | "
               "MA200 4H %s | cloud 4H %s | cloud 15M %s | cloud D1 %s | fib %s | fib D1 %s | "
               "pivots D1 %s | round %s",
               UseMA50_M15 ? "on" : "off", UseMA50_H4 ? "on" : "off", UseMA50_D1 ? "on" : "off",
               UseEMA9_H4 ? "on" : "off", UseEMA9_D1 ? "on" : "off", UseMA200_H4 ? "on" : "off",
               UseCloud_H4 ? "on" : "off", UseCloud_M15 ? "on" : "off", UseCloud_D1 ? "on" : "off",
               UseFib ? "on" : "off", UseFibD1 ? "on" : "off", UseDailyPivots ? "on" : "off",
               UseRoundNumbers ? StringFormat("every %.2f", RoundNumberStep * gPip) : "off");
...
void OnDeinit(const int reason)
  {
   ...
   int hs[12]; hs[0]=hIchiT; hs[1]=hIchiE; hs[2]=hMaT; hs[3]=hEmaT; hs[4]=hMa200T;
   hs[5]=hMaE; hs[6]=hMaD1; hs[7]=hEmaD1; hs[8]=hAtrP; hs[9]=hAtrE; hs[10]=hEmaSide;
   hs[11]=hIchiD1;
   for(int i = 0; i < 12; i++) if(hs[i] != INVALID_HANDLE) IndicatorRelease(hs[i]);
  }
   ...
   if(ok)
     {
      ...
      CopyOne(hIchiD1, 2, 1, gDSenA); CopyOne(hIchiD1, 3, 1, gDSenB);   // v1.10
...
void CollectFib(const ENUM_TIMEFRAMES tf, const int bars, const double weight, const string tag)
  {
   MqlRates r[]; ArraySetAsSeries(r, true);
   int got = CopyRates(_Symbol, tf, 1, bars, r);
   if(got < 10) return;
   int iHi = 0, iLo = 0;
   for(int i = 1; i < got; i++)
     {
   ...
   double hi = r[iHi].high, lo = r[iLo].low;
   int dirUp = (iHi < iLo) ? 1 : -1;       // high more recent -> swing up
   //--- the TrendTF call owns the swing shown on the panel; a second call on
   //    another timeframe must not overwrite it
   if(tf == TrendTF) { gFibHi = hi; gFibLo = lo; gFibDirUp = dirUp; }
   double range = hi - lo;
   if(range <= 0.0) return;
   double ratios[4] = {0.382, 0.5, 0.618, 0.786};
   for(int k = 0; k < 4; k++)
     {
      double p = (dirUp > 0) ? hi - range * ratios[k] : lo + range * ratios[k];
      AddPoint(p, weight, StringFormat("%s %.3f", tag, ratios[k]), 0);
     }
  }

//--- v1.10: psychological levels. Generated around the market rather than
//    from history, and capped at 200 so a silly RoundNumberStep cannot spin.
void CollectRoundNumbers()
  {
   if(!UseRoundNumbers || RoundNumberStep <= 0.0) return;
   double step = RoundNumberStep * gPip;
   if(step <= 0.0) return;
   double mid = 0.5 * (SymbolInfoDouble(_Symbol, SYMBOL_BID) + SymbolInfoDouble(_Symbol, SYMBOL_ASK));
   if(mid <= 0.0) return;
   double span = MaxZoneDistancePips * gPip;
   double p = MathFloor((mid - span) / step) * step;
   for(int n = 0; n < 200 && p <= mid + span; n++, p += step)
      if(p >= mid - span && p > 0.0)
         AddPoint(p, RoundNumberWeight, "round", 0);
  }
...
void RebuildZones(const bool force)
  {
   ...
   if(UseFib) CollectFib(TrendTF, FibSwingBars, FibWeight, "fib");
   //--- v1.10 sources
   if(UseDailyPivots) CollectPivots(PERIOD_D1, DailyPivotLookback, DailyPivotWeight, "piv D1");
   if(UseCloud_D1) { AddPoint(gDSenA, CloudWeight, "kumo D1", 0); AddPoint(gDSenB, CloudWeight, "kumo D1", 0); }
   if(UseFibD1) CollectFib(PERIOD_D1, FibD1SwingBars, FibD1Weight, "fibD1");
   CollectRoundNumbers();
...
void DrawPanel()
  {
   ...
   PanelRow(row++, "SR LEVELS EA v1.10", real ? "REAL MONEY" : "demo", clrGold, real ? clrRed : clrSilver);
   ...
   PanelRow(row++, "ALL TIME", StringFormat("%dW %dL  net %+.2f", gAllWins, gAllLosses, gAllNet),
