...
#property version   "7.12"   // file is TrendEMA_EA_v7.12 - bump both together
...
void OnDeinit(const int reason)
  {
   EventKillTimer();
   if(SingleInstanceLock && !MQLInfoInteger(MQL_TESTER) && gInitOk)
...
void WriteHeartbeat()
  {
   ...
   if(h == INVALID_HANDLE)
      return;
   FileWrite(h, StringFormat("%s|%d|%d",
