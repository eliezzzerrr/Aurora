...
#property version   "1.00"   // file is TrendEMA_EA_v1 - bump both together
...
bool IsNewDay()
  {
   ...
   TimeToStruct(gDayStart,     start);
   return(now.day != start.day || now.mon != start.mon || now.year != start.year);
  }
...
void ResetDayState(const bool firstRun)
  {
   ...
   gLastLossTime    = 0;
   if(!firstRun)
      Print("[TREMA] New trading day. Counters reset.");
  }
