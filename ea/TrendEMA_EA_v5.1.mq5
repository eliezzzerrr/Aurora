...
#property version   "5.10"   // file is TrendEMA_EA_v5.1 - bump both together
...
bool EvaluateRsiExtreme(const bool newEntryBar)
  {
   ...
   if(BlockOpposingEntries && HasOpposingPosition(gXDir))
     {
      SetStatus("RSI-X", "held: opposing position open", clrOrange);
      return(false);
     }
...
void DrawPanel()
  {
   ...
   const int rows  = 15 + (openNow > 0 ? openNow * 2 : 0) + (EnableRsiExtreme ? 1 : 0);
   const int width = 440;
