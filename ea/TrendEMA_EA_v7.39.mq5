...
#property version   "7.39"   // file is TrendEMA_EA_v7.39 - bump both together
...
//--- v7.39: a FOURTH timeframe above TrendTF, read-only by default. See the
//    gate in EvaluateAndAct for what it does and why it starts switched off.
input ENUM_TIMEFRAMES HigherTF  = PERIOD_H1;   // Higher TF shown on the panel (gate is optional)
input bool   RequireHigherTF    = false; // Refuse entries the HigherTF cloud opposes (false = display only)
...
//--- v7.34: chop filter, OFF by default. See the gate in EvaluateAndAct
//    for the reasoning and for why the threshold is deliberately unset.
input double MinCloudPips       = 0;     // Chop filter: min cloud thickness in pips (0 = off)
...
//--- v7.37: DEFAULT FLIPPED TO FALSE. Over 21 months of tester data
//    (2025.01.01 - 2026.09.09, v7.36, 2268 trades, inputs matching live) the
//    15M level was the worst cell in the whole system by a wide margin:
//
//        trigger @ level      n    win%     net R   R/trade
//        TREND @ 15M        317   15.5%    -120.9    -0.381
//        XING  @ 15M         29   13.8%     -13.1    -0.452
//        ---- for contrast ----
//        XING  @ 1M         403   34.2%    +145.6    +0.361
//        TREND @ 1M         444   29.5%     +76.8    +0.173
//        TREND @ 5M         702   27.9%     +72.4    +0.103
//
//    15.5% against a 25% breakeven at 1000/3000. -134R across both 15M cells
//    is about five standard errors - not a slice that happened to look bad.
//    Removing them takes the whole system from 28.0% / +196.9R to
//    30.2% / +330.8R: the only change tested all week that raises the win
//    rate AND the return instead of trading one for the other.
//
//    The mechanism is in the cascade itself. This level is the fallback of
//    the fallback - reached only when the 1M AND 5M levels are both
//    unplaceable, which means price has already run past both. The entry is
//    therefore late, far from current price, and the premise that justified
//    it is usually stale by the time the limit fills.
//
//    v7.38 CORRECTION - v7.37 gated the LIMIT cascade only and made things
//    WORSE. Measured on 2026 100% real ticks, same 1166 trades either way:
//
//                        v7.37 OFF (limit only)      v7.36 baseline
//        TREND @ 15M       0 trades     0.0R          152    -43.7R
//        XING  @ 15M     173 trades   -53.2R           17     -5.1R
//        TOTAL          1166 trades   +85.2R         1166    +93.0R
//        drawdown                     15.95%                 15.46%
//
//    Removing the limit freed the EMA15 slot permanently, and the 15M CROSS
//    - previously blocked by SlotOccupied("EMA15") and, from v7.36, by
//    PendingHoldsSlot("EMA15") - fired 156 extra times and lost back every
//    R the limit removal had saved. The worst cell in the system was
//    shielding a worse one.
//
//    So this input now gates the 15M level for BOTH entry styles. Nothing
//    else uses the EMA15 slot, so with it off those trades simply do not
//    happen rather than being replaced.
//
//    The general lesson, worth keeping: deleting a losing cell from a ledger
//    does NOT predict the result of disabling it. Slots, position caps and
//    the level cascade all interact, and what fills the gap can be worse
//    than what left it. Re-run, never subtract.
input bool   UseFallback15M     = false; // Use the 15M EMA50 as an entry level at all (limit AND cross)
...
//--- v7.29.1: PAUSED on 3 Sep, not deleted. Over 19 resolved trades it ran
//    6/19 = 32% for -268.60, against a realised breakeven of 44% - its
//    average win of 64.00 against an average loss of 50.20 is a ratio of
//    1.27, not the 1.67 the nominal geometry implies, because the barrier
//    keeps shortening targets. It is the only trigger family below its own
//    breakeven; counter-trend sits exactly on 33% against 33.3% and
//    with-trend is 16/37 for +482.00.
//    It is also the least filtered entry in the EA - no trend permission,
//    no RSI arm, no retest gate, no placeable level required - which is why
//    it fires three times as often as the counter it superficially
//    resembles.
//    The statistics do NOT prove it is unprofitable: the 95% interval on
//    6/19 runs from 15% to 54% and contains the 44% it needs. This is a
//    decision on a point estimate with an asymmetric cost, taken because it
//    is reversible in one field and because leaving it running would
//    confound the 08:00-17:00 session filter that has not yet seen a trade.
//    Set true to resume. The backtest is what should settle it.
...
//--- v7.32: 900/1500 was below its own breakeven. Two real-tick runs,
//    XAUUSDc M1, 1 Jun - 1 Sep 2026, 100% history quality, same build:
//
//                        900/1500      1000/3000
//      net profit         -265.89      +1153.27
//      profit factor        0.954         1.197
//      expected payoff      -0.51         +2.62
//      win rate            36.35%        29.55%
//      breakeven needed     37.5%         25.0%
//      max drawdown         9.46%         8.01%
//      trades                 520           440
//
//    Doubling the target costs 6.8 points of win rate and removes 12.5
//    points of breakeven, so the edge survives being projected further -
//    which was the open question. Drawdown falls too, which is not the
//    usual trade for a wider target.
//    The 900/1500 run already carried the v7.31 counter-cross fix and the
//    1000/3000 run did not, so the comparison is biased against the wider
//    geometry and it still wins by ~1400.
//    Cost to be clear about: at 3.0 reward:risk roughly seven trades in ten
//    lose, the backtest showed 22 consecutive losses and an average losing
//    run of 6, with the brake and cooldown already active.
input double SLPips                 = 1000; // SL for every trigger EXCEPT counter-trend (1000 = $10.00)
input double TPPips                 = 3000; // TP for every trigger EXCEPT counter-trend
...
//--- v7.31: counter-trend entries taken by CROSS lose; the same trade taken
//    by LIMIT does not. Real-tick backtest, XAUUSDc M1, 1 Jun - 1 Sep 2026,
//    440 trades:
//        XING-CTR-1M    2/11   -56.13
//        XING-CTR-5M    7/29   -85.40
//        XING-CTR-15M   1/6    -30.24
//        cross counters 10/46  -171.77
//        CTR-1M         5/11   +37.00
//        CTR-5M         2/5    +10.16
//        limit counters  7/16   +47.16
//    All three cross variants lose and both limit variants win, which is
//    three independent buckets agreeing rather than one outlier.
//    It is also mechanically sensible: a counter LIMIT waits at a level for
//    price to come back to it, while a counter CROSS enters at market on a
//    break - fading a trend by chasing it, which is the worst way to take
//    that trade. Only the counter direction is affected; with-trend crosses
//    are the best performer in the same test (XING-1M +738.43).
input bool   AllowCounterCross      = false;// Allow counter-trend entries via a CROSS (limits are unaffected)
...
input bool   PreventDuplicateEntries = true;// Stop two triggers doubling up on the same level
input double DuplicateGuardPips    = 100;   // ...and refuse a market entry this close to a fresh position
input int    DuplicateGuardSeconds = 5;     // ...opened within this many seconds
...
//--- v7.33.1: 6.0 down to 5.0. Swept over the 8-month real-tick run, base
//    +834.97 on this pairing (the test ran at ~0.2% risk per trade because
//    MaxLotSizeCap bound at 2 lots, so thresholds are converted to R and
//    back at the live 0.5%):
//
//      live 3%  =  6R  =  -1.25% in the test  ->  -739.39   fired 37 days
//      live 5%  = 10R  =  -2.0%  in the test  ->   +60.47   fired  3 days
//      live 6%  = 12R  =  -2.5%  in the test  ->     0.00   fired  1 day
//
//    6.0 never triggered once in eight months, which makes it decoration
//    rather than protection. 3.0 fires after about six losing trades - an
//    ordinary day at a 27% win rate - and every threshold that fires that
//    often truncates the distribution and costs more than it saves.
//    5.0 trips roughly four times a year, on days that deserve it.
input double MaxDailyLossPercent    = 5.0;  // Halt for the day at this realised loss % (0 = disabled)
//--- v7.33: a profit ceiling is NOT the mirror of the loss halt. The loss
//    halt caps ruin; this caps the right tail, and at 27% win rate with 3:1
//    geometry the right tail is where the money is. Swept over the 8-month
//    real-tick run (1126 trades, base +834.97 on this pairing):
//
//      +0.50%  -307.60      +1.75%  -226.83
//      +0.75%  -625.06      +2.00%  +214.12
//      +1.00%  -676.02      +2.50%  +264.34
//      +1.25%  -739.39      +3.00%   0.00  (fired twice in 8 months)
//      +1.50%   -82.21      +5.00%   0.00  (never fired)
//
//    Every threshold that fires often enough to matter is harmful, and
//    +1.00% costs 676 of the 835. The mildly positive band at 2-2.5% rests
//    on 21-37 trades and sits inside noise. 5.0 is set because it never
//    triggered once across eight months - it is a stop-for-the-day rule
//    that demonstrably costs nothing on this data, not an edge.
input double MaxDailyProfitPercent  = 5.0;  // Stop for the day at this realised profit % (0 = disabled)
...
input int    CooldownMinutesAfterLoss = 60; // Wait this long once the cooldown applies (0 = no cooldown)
//--- v7.30: a single loss is not information. At a ~40% win rate roughly
//    three trades in five lose, so a one-loss trigger fired after most of
//    them - throttling rather than detecting, and pausing the trend side in
//    a trend after one ordinary stop-out. Two in a row is a pattern worth
//    sitting out; three is MaxSameDirLosses, which stops the side entirely.
//    Set 1 to restore the old behaviour.
input int    CooldownAfterLosses    = 2;  // Consecutive losses on a side before the cooldown applies
input int    MaxSameDirLosses       = 3;    // Block a direction after this many losses in a row (0 = off)
input bool   BrakeReleaseOnReentry  = true; // Also release the brake when the trend breaks and re-forms
...
input string ManualNewsWindows  = "";   // GMT blackout windows, NEEDS BlockNewsWindow (empty = none)
//--- v7.29: a SESSION filter, deliberately independent of BlockNewsWindow.
//    "do not trade around scheduled events" and "do not trade during these
//    hours" are different questions that happened to share a parser: the
//    window list lived inside NewsBlocked(), so switching the calendar gate
//    off on 2 Sep silently disabled clock-based blocking too.
//    Times are EASTERN, not GMT, so the window does not need re-typing when
//    EDT ends - ETOffsetHours already carries the offset for the day
//    rollover and is reused here.
//    Default 08:00-17:00 ET is the US cash session plus the 08:30 data
//    release. Over 60 resolved trades that window ran 2/16 for -409.40
//    while the other fifteen hours ran 20/44 for +497.80, and the decay
//    showed up in all four trigger families rather than one - which is what
//    an environmental effect looks like and a fluke usually does not.
//    Round session boundaries on purpose: a tighter 10:00-17:00 cut scores
//    better on this sample precisely because it excludes the two winners,
//    which is fitting the edges to the data.
input string SessionBlackoutET  = "08:00-17:00"; // ET hours to sit out (empty = none)
...
int      hIchiH  = INVALID_HANDLE;  // v7.39: HigherTF Ichimoku
...
//--- v7.39: HigherTF cloud, kept separate from the TrendTF one above.
double      gSenkouAH   = 0.0;
double      gSenkouBH   = 0.0;
double      gHigherClose = 0.0;
ENUM_CLOUD  gCloudH     = CLOUD_UNKNOWN;
datetime    gLastHigherBar = 0;
...
//--- v7.38.1: gross win/loss this epoch, so the breakeven win rate can be
//    derived from what actually happened rather than from the nominal SL/TP.
double   gAllGrossWin     = 0.0;
double   gAllGrossLoss    = 0.0; // stored POSITIVE
...
bool     gBrakeLeft       = false; // v7.35: cloud has departed gBrakeCloudDir since the brake engaged
...
int OnInit()
  {
   ...
   hIchiH = iIchimoku(_Symbol, HigherTF, IchiTenkan, IchiKijun, IchiSenkou);
   ...
   if(PreventDuplicateEntries)
      PrintFormat("[TREMA] Duplicate guard: ON. A resting limit holds its own "
                  "slot against the cross, and a market entry within %.0f pips "
                  "of a position opened in the last %ds is refused.",
                  DuplicateGuardPips, DuplicateGuardSeconds);
   else
      Print("[TREMA] Duplicate guard: OFF. A limit and a cross can both fill "
            "on the same level.");

   PrintFormat("[TREMA] Counter-trend entries: limits ON, crosses %s.",
               AllowCounterCross ? "ON" : "OFF");

   if(MinCloudPips > 0.0)
      PrintFormat("[TREMA] Chop filter: no entries while the cloud is thinner than "
                  "%.0f pips (%.0f%% of the %.0f-pip stop).",
                  MinCloudPips, 100.0 * MinCloudPips / MathMax(SLPips, 1.0), SLPips);
   else
      Print("[TREMA] Chop filter: OFF (MinCloudPips=0). Cloud thickness is shown "
            "on the panel but gates nothing.");

   if(RequireHigherTF)
      PrintFormat("[TREMA] Higher timeframe: %s cloud must not OPPOSE the %s "
                  "permission. Inside the higher cloud counts as no opinion.",
                  EnumToString(HigherTF), EnumToString(TrendTF));
   else
      PrintFormat("[TREMA] Higher timeframe: %s shown on the panel, gates nothing "
                  "(RequireHigherTF=false).", EnumToString(HigherTF));

   PrintFormat("[TREMA] Session blackout: %s (Eastern, ET offset %+d from GMT)",
               StringLen(SessionBlackoutET) > 0 ? SessionBlackoutET : "OFF",
               ETOffsetHours);

   PrintFormat("[TREMA] Risk brakes: cooldown %s | same-direction brake %s",
               CooldownMinutesAfterLoss > 0
                  ? StringFormat("%d min after %d loss%s in a row, losing side only",
                                 CooldownMinutesAfterLoss, CooldownAfterLosses,
                                 CooldownAfterLosses == 1 ? "" : "es")
                  : "OFF",
               MaxSameDirLosses > 0
                  ? StringFormat("after %d losses in a row on one side, until the 15M flips%s",
                                 MaxSameDirLosses,
                                 BrakeReleaseOnReentry
                                    ? " or the trend breaks and re-forms" : "")
                  : "OFF");
   ...
   PrintFormat("[TREMA] Day rollover %02d:00 ET (GMT%+d).  Daily halt: %s.  "
               ...
               MaxDailyLossPercent > 0.0
                  ? StringFormat("%.1f%% loss / %s profit", MaxDailyLossPercent,
                                 MaxDailyProfitPercent > 0.0
                                    ? StringFormat("%.1f%%", MaxDailyProfitPercent)
                                    : "none")
                  : "DISABLED",
...
void OnDeinit(const int reason)
  {
   ...
   if(hIchiH != INVALID_HANDLE) IndicatorRelease(hIchiH);
...
//+------------------------------------------------------------------+
//--- v7.34: 15M cloud thickness in pips - the gap between Senkou A and B.
//    Reads 0 before the first successful Ichimoku copy, which is why every
//    caller also tests gCloud != CLOUD_UNKNOWN rather than trusting a 0.
double CloudThicknessPips()
  {
   if(gPip <= 0.0)
      return(0.0);
   return(MathAbs(gSenkouA - gSenkouB) / gPip);
  }
...
void EvaluateAndAct(const bool newEntryBar)
  {
   ...
   if(MaxDailyProfitPercent > 0.0 && gDayStartBalance > 0.0 &&
      gDayPL >= MathAbs(gDayStartBalance * MaxDailyProfitPercent / 100.0))
     {
      SetStatus("DONE", StringFormat("daily target  +%.2f / +%.1f%%",
                                     gDayPL, MaxDailyProfitPercent), clrLime);
      CancelPending("daily profit target reached");
      return;
     }
   ...
   //--- v7.29: sits with the other blanket gates, ahead of every trigger,
   //    and independent of the news switch.
   if(InSessionBlackout())
     {
      MqlDateTime e; TimeToStruct(NowET(), e);
      SetStatus("SESSION", StringFormat("blacked out %s ET (now %02d:%02d ET)",
                                        SessionBlackoutET, e.hour, e.min), clrOrange);
      CancelPending("session blackout");
      return;
     }
   ...
   //--- v7.39: HigherTF agreement, OFF by default.
   //
   //    Asked for on 10 Sep after the EA armed a SELL twice into a vertical
   //    rally (09:03 and 09:08, 1M RSI 71.4 then 73.6) while the 15M cloud
   //    still read "below". Worth being clear about what this can and cannot
   //    fix: on that occasion the H1 cloud was ALSO bearish, so this gate
   //    would not have blocked it. The cause there is Ichimoku's own 26-period
   //    forward projection - the cloud price is compared against on TrendTF is
   //    built from data IchiKijun bars back, about 6.5 hours on M15 - so the
   //    permission gate lags a fast move by construction. A second, slower
   //    cloud inherits that lag rather than curing it.
   //
   //    What it CAN do is refuse trades where the two timeframes genuinely
   //    disagree, which is a different and narrower claim, and one nobody has
   //    measured on this system. Hence display-only until swept.
   //
   //    Blocks only when the HigherTF cloud actively OPPOSES the direction.
   //    CLOUD_INSIDE on the higher timeframe is treated as "no opinion" and
   //    allows the trade: price sits inside the H1 cloud a large share of the
   //    time, and refusing everything then would be a far bigger change than
   //    the one being asked for.
   if(RequireHigherTF && gCloudH != CLOUD_UNKNOWN)
     {
      int want = 0;
      if(gCloud == CLOUD_ABOVE)      want =  1;
      else if(gCloud == CLOUD_BELOW) want = -1;
      int higher = (gCloudH == CLOUD_ABOVE) ? 1 : (gCloudH == CLOUD_BELOW ? -1 : 0);
      if(want != 0 && higher != 0 && higher != want)
        {
         SetStatus("BLOCKED", StringFormat("%s cloud opposes (%s vs %s)",
                                           EnumToString(HigherTF),
                                           higher > 0 ? "up" : "down",
                                           want > 0 ? "up" : "down"), clrOrange);
         CancelPending("higher timeframe opposes");
         return;
        }
     }

   //--- v7.34: chop filter. A thin cloud is Senkou A and B converging, which
   //    is the Ichimoku system stating that the trend is undecided - so this
   //    reads the CONFIDENCE of the permission the cloud already grants,
   //    rather than adding an unrelated indicator. gSenkouA/B are copied every
   //    TrendTF bar for the existing above/below/inside test, so this is free.
   //
   //    Motivating day: 4 Sep 2026. Price oscillated in a 1400-pip range for
   //    eleven hours; seven entries fired and all seven took the full stop.
   //    The cloud measured 164-656 pips against a 1000-pip stop and price
   //    crossed in and out of it repeatedly - the panel flipped from "inside"
   //    to "below" and back within eight minutes.
   //
   //    Compared against pips rather than a ratio of SLPips because the two
   //    are set independently and silently coupling them would surprise the
   //    next person to widen the stop. The ratio that matters is still
   //    thickness vs stop, so the startup banner prints it.
   //
   //    DEFAULT 0 = OFF, and it must stay off until a full-period tester run
   //    sets the threshold. It has NOT been swept. A chop filter tuned to a
   //    single bad day will also block the first entry of every good trend,
   //    because a breakout looks exactly like chop until it fires.
   //
   //    Known limitation: Senkou A and B are built from data IchiKijun and
   //    IchiSenkou bars back, so a market that has JUST gone quiet still reads
   //    thick for hours. This catches sustained chop, not its onset.
   //
   //    Cancels a resting limit for the same reason the session blackout does:
   //    a limit left in the book would fill on the very condition being
   //    filtered out.
   if(MinCloudPips > 0.0 && gCloud != CLOUD_UNKNOWN)
     {
      double thickPips = CloudThicknessPips();
      if(thickPips < MinCloudPips)
        {
         SetStatus("BLOCKED", StringFormat("chop filter: cloud %.0fp thin (needs %.0fp)",
                                           thickPips, MinCloudPips), clrOrange);
         CancelPending("cloud thinner than MinCloudPips");
         return;
        }
     }
...
void FireRsiExtreme()
  {
   //--- v7.36: see NearbyPositionExists.
   if(NearbyPositionExists(gXDir, CurrentPrice(gXDir)))
     {
      SetXState("SKIP duplicate of a position opened seconds ago", clrOrange);
      return;
     }
...
void FireCross(const int dir, const string levelTag, const double level,
  ...
  {
   //--- v7.36: see NearbyPositionExists.
   if(NearbyPositionExists(dir, CurrentPrice(dir)))
     {
      SetStatus("SKIP", StringFormat("duplicate: %s cross sits on a position "
                                     "opened seconds ago", levelTag), clrOrange);
      return;
     }

   //--- v7.31: refuse the counter-trend cross outright. Checked before the
   //    barrier test below so the log says WHY it was skipped rather than
   //    blaming the reward:risk.
   if(counter && !AllowCounterCross)
     {
      SetStatus("SKIP", StringFormat("counter cross disabled (%s EMA50)", levelTag),
                clrOrange);
      return;
     }
   ...
   if(gCrossInit && !gCrossBlocked)
     {
      ...
      //--- v7.36: PendingHoldsSlot added alongside SlotOccupied. The slot is
      //    taken by a position OR by our own limit resting on that level.
      if(!deepOnly && x1 && gEma1S > 0.0
         && !SlotOccupied("EMA1") && !PendingHoldsSlot("EMA1"))
        { FireCross(dir, "1M",  gEma1S,  counter); fired = true; }
      else if(!deepOnly && x5 && gEma5S > 0.0
              && !SlotOccupied("EMA5") && !PendingHoldsSlot("EMA5"))
        { FireCross(dir, "5M",  gEma5S,  counter); fired = true; }
      //--- v7.38: UseFallback15M gates the cross too. See the note on the
      //    input for why gating only the limit made things worse.
      else if(UseFallback15M && x15 && gEma15S > 0.0
              && !SlotOccupied("EMA15") && !PendingHoldsSlot("EMA15"))
...
void FireTrendRsi()
  {
   //--- v7.36: see NearbyPositionExists.
   if(NearbyPositionExists(gTDir, CurrentPrice(gTDir)))
     {
      gTState = "SKIP duplicate of a position opened seconds ago";
      SetStatus("TRSI", gTState, clrOrange);
      return;
     }
...
bool RefreshIndicators(const bool force)
  {
   ...
   //--- 15M trend gate ---------------------------------------------------
   //--- v7.39: HigherTF cloud. Read on its own bar clock so an H1 read is
   //    not repeated 60 times an hour. A failure here does NOT touch
   //    gStaleTrend: this is advisory unless RequireHigherTF is on, and a
   //    missing H1 must never be able to halt a system that does not use it.
   datetime hBar = iTime(_Symbol, HigherTF, 0);
   if(force || hBar != gLastHigherBar)
     {
      double hClose[];
      if(CopyClose(_Symbol, HigherTF, 1, 1, hClose) == 1 &&
         CopyOne(hIchiH, 2, 1, gSenkouAH) &&
         CopyOne(hIchiH, 3, 1, gSenkouBH))
        {
         gHigherClose = hClose[0];
         double htop  = MathMax(gSenkouAH, gSenkouBH);
         double hbot  = MathMin(gSenkouAH, gSenkouBH);
         if(gHigherClose > htop)      gCloudH = CLOUD_ABOVE;
         else if(gHigherClose < hbot) gCloudH = CLOUD_BELOW;
         else                         gCloudH = CLOUD_INSIDE;
         gLastHigherBar = hBar;
        }
     }
...
void SaveBrakeState()
  {
   ...
   GlobalVariableSet(BrakeGV("BLEFT"),   gBrakeLeft ? 1.0 : 0.0);
...
void LoadBrakeState()
  {
   ...
   //--- v7.35: absent on state written by <= v7.34.1, and GlobalVariableGet
   //    returns 0 for a missing name, which is the correct default anyway.
   gBrakeLeft     = (GlobalVariableGet(BrakeGV("BLEFT")) > 0.5);
...
void ClearBrakeState()
  {
   ...
   GlobalVariableDel(BrakeGV("BLEFT"));
  }

//--- v7.35: one place that clears the brake, so the two release paths below
//    cannot drift apart in what they reset.
void ReleaseDirBrake(const string why)
  {
   PrintFormat("[TREMA] %s brake RELEASED - %s.",
               gBrakeDir > 0 ? "BUY" : "SELL", why);
   gBrakeDir      = 0;
   gDirLossStreak = 0;
   gDirLossSide   = 0;
   gBrakeLeft     = false;
...
void UpdateDirBrake()
  {
   ...
   //--- A: the permission flipped to the opposite side. The original rule.
   if(now != 0 && now != gBrakeCloudDir)
     {
      ReleaseDirBrake("15M permission flipped");
      return;
     }

   //--- B: v7.35. Rule A alone can only be satisfied by the market going the
   //    OTHER way, so when the brake engages during a definite trend and that
   //    same trend simply resumes, the losing side stays blocked until the
   //    12:00 ET rollover - locked out of exactly the move the EA exists to
   //    trade. On 7 Sep three SELL stops closed within 18 minutes during a
   //    rally; had the cloud been BELOW at that moment rather than INSIDE,
   //    shorts would have been dead for the following seven hours even as
   //    price resumed down.
   //
   //    So: also release once the cloud has LEFT the braked direction and
   //    then come back to it. That is a trend interrupted and re-formed - a
   //    fresh setup - rather than a continuation of the run that just cost
   //    three stops. Rule A is kept because a flip to the opposite side is a
   //    stronger signal still and should release immediately.
   //
   //    Deliberately NOT a timer: a timer lets the same losing direction
   //    resume into unchanged conditions, which is the thing the brake is for.
   //    The cloud has to actually do something.
   //
   //    Nothing to do when gBrakeCloudDir is 0 (the brake engaged while price
   //    was inside the cloud): rule A already releases on any resolution,
   //    because 0 differs from both +1 and -1.
   if(!BrakeReleaseOnReentry || gBrakeCloudDir == 0)
      return;

   if(now != gBrakeCloudDir)
     {
      if(!gBrakeLeft)
        {
         gBrakeLeft = true;
   ...
   if(gBrakeLeft)
      ReleaseDirBrake("trend broke and re-formed");
  }
...
void NoteClosedTrade(const int posDir, const double net)
  {
   //--- v7.30: the streak is now read by the COOLDOWN as well, so it has to
   //    be maintained whether or not the brake is enabled. Only the brake
   //    engagement below is gated on MaxSameDirLosses.
   if(posDir == 0)
      return;
   ...
   if(MaxSameDirLosses > 0 && gDirLossStreak >= MaxSameDirLosses && gBrakeDir != posDir)
     {
      gBrakeDir      = posDir;
      gBrakeCloudDir = TDirFromCloud();
      gBrakeLeft     = false;   // v7.35: a fresh brake has not been interrupted yet
...
bool InCooldown(const int dir)
  {
   ...
   //--- v7.30: hold off until the losses form a run on this side. The streak
   //    is in-memory (persisted to a GlobalVariable) while the time and side
   //    are rebuilt from deal history, so on the very first run after a
   //    fresh install the streak reads 0 and the cooldown simply does not
   //    apply until two losses occur under this instance. Erring toward not
   //    blocking is the right way round for a gate that only ever costs
   //    trades.
   if(CooldownAfterLosses > 1)
     {
      if(gDirLossSide != gLastLossDir || gDirLossStreak < CooldownAfterLosses)
         return(false);
     }
...
//--- shared window matcher so the two blackout inputs cannot drift apart
   ...
   for(int i = 0; i < n; i++)
     {
      ...
      //--- a window may wrap midnight, e.g. 22:00-02:00
...
datetime NowET()
  {
   return(TimeGMT() + (datetime)(ETOffsetHours * 3600));
  }

//--- the session gate. Independent of BlockNewsWindow by design.
bool InSessionBlackout()
  {
   return(InWindowList(SessionBlackoutET, NowET()));
  }

bool InManualNewsWindow()
  {
   return(InWindowList(ManualNewsWindows, TimeGMT()));
  }
...
//--- v7.36: our OWN resting limit is a claim on a slot that SlotOccupied
//    cannot see, because it scans PositionsTotal() and a pending order is
//    not a position. That blind spot is what let a TREND limit and a XING
//    cross both fill on the 5M EMA50 at 10:31:40.671 on 8 Sep - 0.11 lots
//    twice, 9 pips apart, two of the three concurrent slots spent on one
//    signal. Both comments map to the same slot via TriggerSlot, so the
//    rule INTENDED to prevent it; it just could not see the order.
//
//    Scoped deliberately to our own tracked limit at the same level rather
//    than "any pending order occupies its slot". The broad version would
//    suppress the cross trigger almost permanently, since a TREND limit
//    rests at the entry level nearly all the time, and XING-1M was the
//    single best trigger in the 8-month test at +738.43. Trading that away
//    to fix seven events would be a bad bargain.
//
//    Honest cost: when the limit is resting at the same level and price
//    crosses WITHOUT trading down to the limit price, the cross is now
//    skipped and no entry happens at all. Over the 8-month record the
//    collision fired 7 times in 1126 entries, so the exposure is small,
//    but this has NOT been swept and the number of crosses it suppresses
//    is unmeasured.
bool PendingHoldsSlot(const string slot)
  {
   if(!PreventDuplicateEntries || gPendingTicket == 0)
      return(false);
   if(!PendingAlive(gPendingTicket))
      return(false);
   return(SlotForLevel(gPendingLevelSrc) == slot);
  }

//--- v7.36: a second line of defence for the market-order triggers, which
//    unlike a resting limit CAN be vetoed at the moment of sending. Refuses
//    an entry that would sit on top of a position opened moments ago in the
//    same direction.
//
//    Thresholds come from the seven observed collisions, not from taste:
//    the largest price gap was 65 pips and the largest time gap 1 second,
//    so 100 pips / 5 seconds covers every one with room and is far too
//    narrow to touch a genuine second entry at a different level - the
//    closest legitimate pair on 8 Sep was 720 pips apart.
bool NearbyPositionExists(const int dir, const double price)
  {
   if(!PreventDuplicateEntries || DuplicateGuardPips <= 0.0 || dir == 0)
      return(false);
   datetime cutoff = TimeCurrent() - DuplicateGuardSeconds;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ulong t = PositionGetTicket(i);
      if(t == 0) continue;
      ...
      if((datetime)PositionGetInteger(POSITION_TIME) < cutoff) continue;
      if(MathAbs(PositionGetDouble(POSITION_PRICE_OPEN) - price)
         <= DuplicateGuardPips * gPip)
         return(true);
     }
   return(false);
  }
...
double ConfigSignature()
  {
   string sig = StringFormat(
      ...
      MinCloudPips + (RequireHigherTF ? 100000.0 : 0.0));
...
void DrawPanel()
  {
   ...
   //--- v7.39: the HigherTF read, above the TrendTF row so the panel reads
   //    top-down slowest-to-fastest like the chart does.
   string hTxt; color hClr;
   switch(gCloudH)
     {
      case CLOUD_ABOVE:  hTxt = "UP   above cloud"; hClr = clrLime;   break;
      case CLOUD_BELOW:  hTxt = "DOWN below cloud"; hClr = clrTomato; break;
      case CLOUD_INSIDE: hTxt = "NONE inside cloud";hClr = clrGray;   break;
      default:           hTxt = "loading...";       hClr = clrGray;   break;
     }
   PanelRow(r++, StringFormat("%s CLOUD", EnumToString(HigherTF)),
            StringFormat("%s  %s/%s%s", hTxt,
                         DoubleToString(MathMin(gSenkouAH, gSenkouBH), 2),
                         DoubleToString(MathMax(gSenkouAH, gSenkouBH), 2),
                         RequireHigherTF ? "  GATING" : "  (advisory)"),
            PanelTextColor, hClr);
   ...
   //--- v7.39: the CHOPPY MARKET row is gone at the user's request - it was
   //    advisory only and the panel needed the line for the HigherTF cloud.
   //    MinCloudPips itself is untouched; when it is armed and blocking, the
   //    STATUS row names it ("chop filter: cloud NNNp thin"), so nothing that
   //    affects a trading decision is now invisible. CloudThicknessPips() is
   //    still used by that gate.
   ...
   //--- v7.28: this row printed the NOMINAL cap and never consulted the
   //    barrier, so on a counter setup it read "TP 1000" while the trade it
   //    described would be refused outright. On 2 Sep the 15M EMA50 sat 370
   //    pips above a counter BUY entry - 320 after the buffer, RR 0.64
   //    against a 1.50 floor - and the panel still advertised 1000. Show
   //    what the trade would actually get, and say when that is nothing.
   double  shownLevel = (gEntryLevel > 0.0) ? gEntryLevel : gEma1S;

   string tpTxt = "-";
   color  modeClr = !haveDir ? clrGray : (dir > 0 ? clrLime : clrTomato);
   if(haveDir)
     {
      double useSL  = counter ? CounterSLPips : SLPips;
      double useTP  = counter ? TakeProfitCounterPips : TPPips;
      double floorRR = counter ? MinCounterRR : 0.0;
      double barrier = 0.0, room = 0.0;
      if(counter && CounterUseBarrierTP && shownLevel > 0.0)
         useTP = CounterTargetPips(dir, shownLevel, barrier, room);

      double rr = (useSL > 0.0) ? useTP / useSL : 0.0;
      bool   refused = (floorRR > 0.0 && rr < floorRR);
      tpTxt = StringFormat("TP %.0f  RR %.2f%s", useTP, rr,
                           refused ? StringFormat("  REFUSED < %.2f", floorRR) : "");
      if(refused)
         modeClr = clrOrange;
     }
   PanelRow(r++, "MODE", modeTxt + "   " + tpTxt, PanelTextColor, modeClr);
   PanelRow(r++, "LIMIT @",
   ...
   //--- v7.38.1: the win rate on its own says nothing - 28% is excellent at
   //    1000/3000 and ruinous at 1000/1500. What matters is the margin over
   //    BREAKEVEN, so show the number that has to be beaten.
   //
   //    Derived from realised results (gross win per win vs gross loss per
   //    loss) rather than from SLPips/TPPips, because the nominal 25.0% is a
   //    FLOOR the system never actually runs at: counter-trend entries use
   //    500/1000 (33.3% breakeven) and both counter and RSI-X targets get
   //    shortened to a structural barrier. On the 2026 real-tick record the
   //    blended figure was 26.0%, not 25.0% - small, but it is the difference
   //    between "at breakeven" and "below it".
   //
   //    Falls back to the geometry figure until at least one win AND one loss
   //    exist, since the ratio is undefined before then. A handful of trades
   //    makes it noisy either way; it settles as the epoch fills.
   double beRate, avgW = 0.0, avgL = 0.0;
   bool   beFromResults = (gAllWins > 0 && gAllLosses > 0 && gAllGrossLoss > 0.0);
   if(beFromResults)
     {
      avgW   = gAllGrossWin  / gAllWins;
      avgL   = gAllGrossLoss / gAllLosses;
      beRate = 100.0 / (1.0 + avgW / avgL);
     }
   else
      beRate = 100.0 * SLPips / MathMax(SLPips + TPPips, 1.0);

   int    beN   = gAllWins + gAllLosses;
   double nowWR = (beN > 0) ? 100.0 * gAllWins / beN : 0.0;
   double marg  = nowWR - beRate;
   string beTxt;
   color  beClr;
   if(beN == 0)
     {
      beTxt = StringFormat("need %.1f%% (geometry) - no closed trades yet", beRate);
      beClr = clrGray;
     }
   else
     {
      beTxt = StringFormat("need %.1f%%%s   now %.1f%%   %+.1f pts",
                           beRate, beFromResults ? "" : " (geometry)", nowWR, marg);
      beClr = (marg >= 0.0) ? clrLime : clrTomato;
     }
   ...
   //--- v7.39: this row was off by one bar and said so in the most misleading
   //    possible direction. gLastTrendBar / gLastSignalBar / gLastEntryBar hold
   //    iTime(TF, 0) - the FORMING bar - because that is what the change
   //    detector compares against. But every indicator is read at shift 1, so
   //    the values on the panel came from the bar BEFORE the one this row
   //    named. At 01:13 it displayed "15M 01:00" while the cloud verdict was
   //    computed from the bar that opened at 00:45.
   //
   //    That made the panel look MORE current than it was, which is the worst
   //    way for a diagnostic row to be wrong: it was reported on 10 Sep as
   //    "the cloud is already up, why is this not updated" when the EA was
   //    behaving correctly and only the label lied.
   //
   //    Subtracting one period from the tracked value rather than calling
   //    iTime(TF, 1) fresh, so that if a read FAILED the row keeps showing the
   //    bar actually used instead of silently advancing.
   PanelRow(r++, "BARS USED",
            StringFormat("15M %s   5M %s   1M %s",
                         TimeToString(gLastTrendBar  - PeriodSeconds(TrendTF),  TIME_MINUTES),
                         TimeToString(gLastSignalBar - PeriodSeconds(SignalTF), TIME_MINUTES),
                         TimeToString(gLastEntryBar  - PeriodSeconds(EntryTF),  TIME_MINUTES)),
            PanelTextColor, clrDeepSkyBlue);
   ...
   //    closed position, or a COOLDOWN row that expires, leaves its labels
   //    on the chart and everything below shifts up underneath them.
   //--- v7.27.4: this has to run AFTER the last PanelRow, not in the middle.
   //    v7.27.3 moved the health block below it, so gPanelRowsDrawn recorded
   //    a count that stopped at TODAY and the five health rows were never
   //    eligible for cleanup - which is why SPREAD drew twice once the panel
   //    shrank by a row.
   ...
   if(value != "")
     {
      ...
      //--- v7.29: an empty value used to just skip the object, so whatever
      //    that row held last time stayed on the chart. When the panel lost
      //    a row and a separator slid up into a slot that had carried TODAY,
      //    the old TODAY value kept drawing beside it - two TODAY lines with
      //    different P/L. The row-count cleanup could not catch it because
      //    the row still exists, it just has nothing to show.
