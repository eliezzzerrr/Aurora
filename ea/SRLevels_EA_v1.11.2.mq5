...
#property version   "1.112"  // file is SRLevels_EA_v1.11.2 - bump both together
...
//--- v1.11.2, 15 Sep 2026: MIDNIGHT ET, not 17:00. 17:00 was the forex day
//    boundary and it was the default from v1.0, but at 03:51 local (GMT+8)
//    the operator saw DAY CAP 3/3 still holding from the previous session
//    and expected a fresh day. Midnight ET is the boundary they think in.
//    Consequence to know: the day now rolls at noon local time, so a heavy
//    morning session and the afternoon that follows it share one daily cap
//    and one 5% loss halt, where before the split fell at 05:00 local.
//    TrendEMA rolls at 12:00 ET and is deliberately left alone.
input int    DayResetHourET     = 0;     // Hour (ET) the trading day rolls over (0 = midnight ET)
...
void DrawPanel()
  {
   ...
   PanelRow(row++, "SR LEVELS EA v1.11.2", real ? "REAL MONEY" : "demo", clrGold, real ? clrRed : clrSilver);
