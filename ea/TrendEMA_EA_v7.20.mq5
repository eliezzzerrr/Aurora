...
//
//  RISK
//    SL 900 / TP 2000 with-trend. Counter-trend keeps 500 / 1000.
...
#property version   "7.20"   // file is TrendEMA_EA_v7.20 - bump both together
...
input double CrossSLPips        = 900;   // SL for cross (breakout/breakdown) entries
input double CrossTPPips        = 2000;  // TP for cross entries
...
input double RsiXStopLossPips   = 900;   // SL for extreme trades
input double RsiXTakeProfitPips = 2000;  // TP for extreme trades
...
input double MinRsiXRR          = 1.0;   // Skip the washout below this reward:risk
...
input double TrendRsiSLPips     = 900;   // SL for trend-RSI trades
input double TrendRsiTPPips     = 2000;  // TP for trend-RSI trades
...
input group "=== RISK ==="
input double StopLossPips           = 900;  // SL distance (900 = $9.00)
input double TakeProfitTrendPips    = 2000; // TP with-trend (2000 = $20.00)
...
int OnInit()
  {
   ...
   if(RsiXUseBarrierTP)
      PrintFormat("[TREMA] RSI extreme also targets the first 15M barrier, "
                  "REFUSED below RR %.2f.", MinRsiXRR);

   if(CounterUseBarrierTP)
      PrintFormat("[TREMA] Counter-trend targets the first 15M barrier "
   ...
   if(EnableTrendRsi)
      PrintFormat("[TREMA] TREND RSI: ENABLED  SELL above %.0f / BUY below %.0f  "
                  ...
                  TrendRsiSLPips, TrendRsiTPPips, TrendRsiExpiryBars,
...
void FireRsiExtreme()
  {
   ...
   double tpPips = RsiXTakeProfitPips;
   if(RsiXUseBarrierTP)
     {
      double barrier = 0.0, room = 0.0;
      tpPips = BarrierTargetPips(gXDir, entryNow, RsiXTakeProfitPips,
                                 barrier, room);
      double rr = (RsiXStopLossPips > 0.0) ? tpPips / RsiXStopLossPips : 0.0;
      if(rr < MinRsiXRR)
        {
         PrintFormat("[TREMA] RSI EXTREME %s REFUSED: only %.0f pips to the 15M "
                     "barrier at %.2f (entry %.2f), RR %.2f below the %.2f floor.",
                     gXDir > 0 ? "BUY" : "SELL", room, barrier, entryNow,
                     rr, MinRsiXRR);
...
void FireCross(const int dir, const string levelTag, const double level,
               const bool counter)
  {
   double slPips = counter ? CounterSLPips         : CrossSLPips;
   double tpPips = counter ? TakeProfitCounterPips : CrossTPPips;
   ...
   if(counter)
     {
      ...
      if(rr < MinCounterRR)
        {
         PrintFormat("[TREMA] Counter-trend CROSS %s REFUSED: only %.0f pips to "
                     "the 15M barrier at %.2f, RR %.2f below the %.2f floor.",
                     dir > 0 ? "BUY" : "SELL", room, barrier, rr, MinCounterRR);
...
void ManageLimit(const int dir, const bool counter, const bool newEntryBar)
  {
   ...
   double slPips = counter ? CounterSLPips        : StopLossPips;
   ...
   if(counter)
     {
      ...
      if(rr < MinCounterRR)
        {
         SetStatus("SKIP", StringFormat("counter %.0fp to barrier, RR %.2f < %.2f",
                                        room, rr, MinCounterRR), clrOrange);
         PrintFormat("[TREMA] Counter-trend %s REFUSED: only %.0f pips of room to "
                     "the 15M barrier at %.2f (entry %.2f), RR %.2f below the "
                     "%.2f floor.", dir > 0 ? "BUY" : "SELL", room, barrier,
                     level, rr, MinCounterRR);
...
//+------------------------------------------------------------------+
//--- first 15M structure price meets moving AWAY from a counter entry.
//    Selling from above: the highest of cloud-top / EMA50 that is below.
//    Buying from below : the lowest of cloud-bottom / EMA50 that is above.
...
double CounterBarrier(const int dir, const double entry)
  {
   ...
   if(dir < 0)
     {
      double best = 0.0;
      if(cloudTop > 0.0 && cloudTop < entry) best = MathMax(best, cloudTop);
      if(gEma15S  > 0.0 && gEma15S  < entry) best = MathMax(best, gEma15S);
      return(best);
     }

   double best = 0.0;
   if(cloudBot > 0.0 && cloudBot > entry) best = cloudBot;
   if(gEma15S  > 0.0 && gEma15S  > entry)
      best = (best > 0.0) ? MathMin(best, gEma15S) : gEma15S;
   return(best);
  }
...
bool RefreshIndicators(const bool force)
  {
   ...
   bool     entryTick = (eBarNow != gLastEntryBar);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      double slP, tpP;
      if(isX)             { slP = RsiXStopLossPips;  tpP = RsiXTakeProfitPips;  }
      else if(isT)        { slP = TrendRsiSLPips;    tpP = TrendRsiTPPips;      }
      else if(isCtr)      { slP = CounterSLPips;     tpP = TakeProfitCounterPips; }
      else if(isC)        { slP = CrossSLPips;       tpP = CrossTPPips;         }
      else                { slP = StopLossPips;      tpP = TakeProfitTrendPips; }
      ...
      //--- v7.19: a counter-trend TP is now computed from live structure at
      ...
      if(isX && RsiXUseBarrierTP && curTP > 0.0)
         wantTP = curTP;   // same reason: computed from live structure at entry
      ...
      if(gTrade.PositionModify(t, wantSL, wantTP))
        ...
        {
         gStopFixFails++;          // only FAILURES consume the budget
...
double ConfigSignature()
  {
   string sig = StringFormat(
      ...
      "%d|%d|%d|%d|%d|%d|%d",
      STRATEGY_REV,
      StopLossPips, TakeProfitTrendPips, TakeProfitCounterPips, CounterSLPips,
      CounterUseBarrierTP ? 1.0 : 0.0, CounterBarrierBuffer, MinCounterRR,
      RsiXUseBarrierTP ? 1.0 : 0.0, MinRsiXRR,
      CrossSLPips, CrossTPPips, RsiXStopLossPips, RsiXTakeProfitPips,
   ...
   if(ShowSLTPLines)
     {
      ...
      double slP = counter ? CounterSLPips : StopLossPips;
