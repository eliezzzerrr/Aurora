...
#property version   "1.52"   // file is TripleEMA_Trend_v1.5.2 - bump both together
...
void PanelLabel(const string name, const int x, const int y, const string text, const color clr)
  {
   ...
   //--- v1.5.2: the cap was a flat 64 characters, which cut live rows in the
   //    middle of a word - the ARM row reads "TOUCH armed - buy the moment
   //    price dips to EMA9 4363.94 (wick counts)" at 69 characters and was
   //    displayed as "...4363.94 (wick...". The cap exists so one long string
   //    cannot stretch the panel off the chart, but 64 was well under the
   //    longest row the EA actually prints (the BLOCKED status detail runs to
   //    about 79). So: budget 88 characters, and shrink that to whatever the
   //    chart is actually wide enough for, measured in the panel's own font.
   //    Consolas is monospaced, so one character measurement sizes them all.
   string shown = text;
   int budget = 88;
   int chartW = (int)ChartGetInteger(0, CHART_WIDTH_IN_PIXELS);
   uint cw = 0, chh = 0; TextGetSize("0", cw, chh);
   if(chartW > 0 && cw > 0)
     {
      int fits = (chartW - x - 16) / (int)cw;
      if(fits < budget) budget = fits;
      if(budget < 24)   budget = 24;      // a panel this narrow is unreadable either way
     }
   if(StringLen(shown) > budget) shown = StringSubstr(shown, 0, budget - 3) + "...";
