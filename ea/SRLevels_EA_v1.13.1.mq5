...
#property version   "1.131"  // file is SRLevels_EA_v1.13.1 - bump both together
...
void DrawPanel()
  {
   ...
   PanelRow(row++, "SR LEVELS EA v1.13.1", real ? "REAL MONEY" : "demo", clrGold, real ? clrRed : clrSilver);
   ...
   if(LineActive() && (gTrendDir == 1 || gTrendDir == -1))
     {
      ...
      PanelRow(row++, "INVALIDATION", StringFormat("%s EMA%d %.2f   price %.2f %s   %s",
               TfName(TrendEMATF), TrendEMAPeriod, gSideEma, px,
               px > gSideEma ? "ABOVE" : (px < gSideEma ? "BELOW" : "AT"),
               beyond ? StringFormat("%s INVALIDATED since %s  (no %s beyond the line)", DirName(gTrendDir),
                                     TimeToString(gBeyondSince, TIME_MINUTES), gTrendDir < 0 ? "sells" : "buys")
                      : StringFormat("%s valid  (%s only %s the line)", DirName(gTrendDir),
                                     gTrendDir < 0 ? "sells" : "buys", gTrendDir < 0 ? "below" : "above")),
