...
#property version   "7.381"  // file is TrendEMA_EA_v7.38.1 - bump both together
...
void DrawPanel()
  {
   ...
   //--- v7.34.1: the chop verdict, as its own row. v7.34 tacked the thickness
   //    onto the cloud row above; a dedicated row keeps the number in one
   //    place and answers the question directly instead of making the reader
   //    compare two figures.
   //
   //    The reference differs by design. With MinCloudPips set, the row says
   //    what the FILTER thinks and whether it is blocking right now. With the
   //    filter off it still answers, measured against SLPips, because a cloud
   //    narrower than the stop means the whole zone of indecision fits inside
   //    the risk being taken - which is what went wrong on 4 Sep. It is
   //    labelled "advisory" in that case so the row is never mistaken for the
   //    filter being armed.
   //
   //    Thickness alone does not capture everything chop does. On 4 Sep the
   //    cloud verdict itself flipped from inside to below and back within
   //    eight minutes; counting those flips would read the churn more directly
   //    but needs history this row does not keep. Read this as one indicator,
   //    not a verdict.
   string chopTxt;
   color  chopClr;
   if(gCloud == CLOUD_UNKNOWN)
     {
      chopTxt = "-  loading";
      chopClr = clrGray;
     }
     ...
     {
      double thickNow = CloudThicknessPips();
      double refPips  = (MinCloudPips > 0.0) ? MinCloudPips : SLPips;
      bool   choppy   = (thickNow < refPips);
      chopTxt = StringFormat("%s  cloud %.0fp %s %.0fp%s",
                             choppy ? "YES" : "no ",
                             thickNow,
                             choppy ? "<" : ">=",
                             refPips,
                             (MinCloudPips > 0.0)
                                ? (choppy ? "  BLOCKING" : "")
                                : "  stop (advisory)");
      chopClr = choppy ? clrTomato : clrLime;
     }
   PanelRow(r++, "CHOPPY MARKET", chopTxt, PanelTextColor, chopClr);
