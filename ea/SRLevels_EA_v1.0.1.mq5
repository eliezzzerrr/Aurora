...
#property version   "1.01"   // file is SRLevels_EA_v1.0.1 - bump both together
...
//    2026 takes it back. The build with the next-zone target is one field
//    away: TPMode=TP_NEXT_ZONE, MinRR=0.6, SLMode=SL_NEXT_LEVEL,
//    MaxSLPips=2500.
...
input ENUM_TP_MODE TPMode       = TP_FIXED_RR;   // Where the target goes
...
input double FixedRR            = 3.0;   // TP_FIXED_RR multiple, also the fallback
...
void DrawPanel()
  {
   ...
   PanelRow(row++, "SR LEVELS EA v1.0.1", real ? "REAL MONEY" : "demo", clrGold, real ? clrRed : clrSilver);
