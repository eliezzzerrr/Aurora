...
#property version   "1.10"   // file is SRLevels_EA_v1.1 - bump both together
...
input ENUM_TREND_MODE TrendMode = TREND_CLOUD_AND_MA; // How the 4H picks a side
...
//    The 3R build is four fields away: TPMode=TP_FIXED_RR, FixedRR=3.0,
//    SLMode=SL_ZONE, MaxSLPips=1000.
input ENUM_SL_MODE SLMode       = SL_NEXT_LEVEL; // Where the stop goes
...
input double MaxSLPips          = 2500;  // Refuse the trade if the stop would be wider than this
...
input double MinRR              = 0.6;   // Refuse a level target below this reward:risk (level modes only)
input double FixedRR            = 1.5;   // TP_FIXED_RR multiple, also the fallback when no level is in range
...
input bool   BlockOpposingEntries = true;// Never hold a BUY and a SELL at once
...
int OnInit()
  {
   ...
   PrintFormat("[SRL] Initialised. Symbol=%s Digits=%d Pip=%.5f  Trend=%s on %s  Entry=%s  "
               ...
               MinSLPips, MaxSLPips, TPModeName(), MinRR,
...
bool Geometry(const int dir, const double entry, const int t, const Zone &z,
              double &sl, double &tp, string &why)
  {
   ...
   if(TPMode == TP_FIXED_RR)
     { tpRaw = entry + dir * FixedRR * dist; tgt = "fixed"; }
   else
     {
      int zi = -1;
...
void DrawPanel()
  {
   ...
   PanelRow(row++, "SR LEVELS EA v1.1", real ? "REAL MONEY" : "demo", clrGold, real ? clrRed : clrSilver);
