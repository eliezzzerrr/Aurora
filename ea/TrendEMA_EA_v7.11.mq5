...
//
//  RISK
//    SL 900 pips ($9.00) fixed. TP 2000 with-trend / 1800 counter.
...
#property version   "7.11"   // file is TrendEMA_EA_v7.11 - bump both together
...
input double TakeProfitCounterPips  = 1800; // TP counter-trend (1800 = $18.00)
...
double ConfigSignature()
  {
   string sig = StringFormat(
      "%s|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%d|%.1f|%.1f|%.1f|%d|"
      "%d|%.1f|%.1f|%.1f|%.1f|%d|%d|%d|%.1f|%.1f|%d|%d|%d|%.0f|%.0f|%d|%d|%d|"
      "%d|%d|%d|%d|%d|%d|%d",
      STRATEGY_REV,
      StopLossPips, TakeProfitTrendPips, TakeProfitCounterPips,
