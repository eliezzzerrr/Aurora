...
#property version   "7.21"   // file is TrendEMA_EA_v7.21 - bump both together
...
input string ManualNewsWindows  = "12:15-13:15,14:00-14:30"; // GMT blackout windows (empty = none)
         ...
         if(rkz != gLastRejectKey)
           {
            ...
            PrintFormat("[TREMA] RSI EXTREME %s REFUSED: only %.0f pips to the "
                        "15M barrier at %.2f (entry %.2f), RR %.2f below the "
                        "%.2f floor.", gXDir > 0 ? "BUY" : "SELL", room,
                        barrier, entryNow, rr, MinRsiXRR);
           }
         ...
         if(rkx != gLastRejectKey)
           {
            ...
            PrintFormat("[TREMA] Counter-trend CROSS %s REFUSED: only %.0f pips "
                        "to the 15M barrier at %.2f, RR %.2f below the %.2f "
                        "floor.", dir > 0 ? "BUY" : "SELL", room, barrier,
                        rr, MinCounterRR);
           }
         ...
         if(rk != gLastRejectKey)
           {
            ...
            PrintFormat("[TREMA] Counter-trend %s REFUSED: only %.0f pips of room "
                        "to the 15M barrier at %.2f (entry %.2f), RR %.2f below "
                        "the %.2f floor.", dir > 0 ? "BUY" : "SELL", room,
                        barrier, level, rr, MinCounterRR);
           }
...
void ProbeCalendar()
  {
   ...
   PrintFormat("[TREMA] NEWS PROBE: %d USD events in the next 24h, %d of them "
               "high-impact. Calendar gate is live.", n, high);
  }
