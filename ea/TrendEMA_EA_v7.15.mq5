...
#property version   "7.15"   // file is TrendEMA_EA_v7.15 - bump both together
...
void OnTimer()
  {
   ...
   TouchLock();
   WriteHeartbeat();
   //--- indicator reads are bar-gated internally, so this is cheap
   //--- v7.15: do NOT stamp gPhase here. OnTimer overwriting it with TIMER
   //    every second erased whatever phase OnTick was in, so the heartbeat's
   //    crash locator always read TIMER and pointed nowhere. The timer's own
   //    work is cheap and well-tested; the tick path is what dies.
   ...
   if(gCrossInit && !gCrossBlocked)
     {
      ...
      if(x1 && gEma1S > 0.0 && !SlotOccupied("EMA1"))
        { FireCross(dir, "1M", gEma1S);  fired = true; }
      else if(x5 && gEma5S > 0.0 && !SlotOccupied("EMA5"))
        { FireCross(dir, "5M", gEma5S);  fired = true; }
      else if(x15 && gEma15S > 0.0 && !SlotOccupied("EMA15"))
        { FireCross(dir, "15M", gEma15S); fired = true; }
     }
...
void SyncPositionStops()
  {
   ...
   if(gLastStopSyncMs != 0 && (nowSync - gLastStopSyncMs) < 2000)
      return;
...
double ConfigSignature()
  {
   string sig = StringFormat(
      "%s|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%.0f|%d|%.1f|%.1f|%.1f|%d|"
